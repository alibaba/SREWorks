...
class SRESearch extends React.Component {
  ...
  componentDidMount() {
    ...
    setTimeout(
      function () {
        $(`.biz-tSearch-deskstop.${this.props.className} input`)
          .css({ width: includeSearchBtn ? '100%' : '484px' })
          .after('<i class="anticon anticon-search ant-input-search-icon"></i>')
      }.bind(this),
      0,
    )
  }
  getAllSuggetPathList = (value) => {
    ...
    appService
      .desktopKeySearch(userEmpId, category, textValue, this.navType)
      .then((res) => {
  ...
  render() {
    ...
    return (
      <div
        ...
        className={classNames('biz-tSearch-deskstop', this.props.className)}
      ...
      >
        ...
        <div
          style={{
            display: hotKeyWordsVisible && hotKeyOptions.length ? '' : 'none',
            position: 'absolute',
          }}
          className="panel-container globalBackground"
        >
          {textValue ? (
          ...
          <div className="hotKey-panel">
            {hotKeyOptions.map((kw, index) => {
              return (
                <div
                  className="hotKeyItem"
                  key={index}
                  onClick={(e) => this.onSearchKeyword(kw.content)}
                >
                  <Badge
                    offset={[8, -2]}
                    count={index + 1}
        ...
        <div
          style={{ display: overlayVisible ? '' : 'none', position: 'absolute' }}
