...
import { Dropdown, Menu, Button, Tooltip, Divider } from 'antd'
...
export default class DefaultItemToolbar extends React.Component {
  ...
  handleActionClick = (action) => {
    let { openAction, callback, nodeParams, itemData, userParams } = this.props,
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                <RunnableAction
                  ...
                  type={'link'}
                  size={size}
                  paramsSet={Object.assign({}, userParams, params, nodeParams, itemData)}
  ...
  render() {
    ...
    let { customRender } = itemToolbar,
      paramsSet = Object.assign({}, userParams, nodeParams, itemData)
    return (
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
        {customRender && (
          <JSXRender
            jsx={util.renderTemplateString(customRender, { row: paramsSet, ...paramsSet })}
          />
        )}
...
const RunnableAction = ({
  ...
  if (type === 'link')
    return (
      <a
        className="oam-action-bar-item"
        onClick={() => openAction && openAction(action, paramsSet, callback)}
      >
        {icon ? <LegacyIcon type={icon} style={{ marginRight: 6 }} /> : null}
        {label}
      </a>
