...
const scriptIndex = args.findIndex(x => x === 'build' || x === 'create' || x === 'update' || x === 'start' || x === 'test' || x==='create-vue');
...
switch (script) {
  ...
  case 'create-vue': {
    const result = spawn.sync(
      'node',
      nodeArgs.concat(require.resolve('../scripts/' + 'create-vue')).concat(args.slice(scriptIndex + 1)),
      { stdio: 'inherit' },
    );
    if (result.signal) {
      if (result.signal === 'SIGKILL') {
