...
import { Button, Card, Tooltip, Drawer, message } from 'antd'
...
import { PopoverConfirm, JsonEditor } from '@sreworks/components'
...
export default class WidgetHandleCard extends React.Component {
  ...
  onCopy = () => {
    ...
    let textWidgetModel =
      typeof replaceUniquekeyWidgetModel === 'string'
        ? replaceUniquekeyWidgetModel
        : JSON.stringify(replaceUniquekeyWidgetModel)
    try {
  ...
  handleCopyCreate = () => {
    let { handleKey, nodeModel } = this.props,
      { type, config } = this.copyWidgetJson
    if (!type || !config) {
  ...
  render() {
    let { visible, selectKey, prevKey, widgetModel, prevModel, widgetMeta } = this.state,
      { cardHeight, exclude, include, filterType } = this.props
    let editTitle = ''
    ...
    return (
      <Card
        ...
        bodyStyle={{
          ...
          padding: selectKey === 'paste' ? '3px 0px' : undefined,
        }}
        tabList={[
          {
            key: 'new',
