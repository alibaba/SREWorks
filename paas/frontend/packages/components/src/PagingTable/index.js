...
import { localeHelper, util, httpClient } from '@sreworks/shared-utils'
...
class PagingTable extends Component {
          ...
          showTotal: (total) => {
            let extFooter = (
              <div>
                ...
                <div>{localeHelper.get('manage.trip', '共 {count} 条', { count: total })}</div>
              </div>
            )
            return extFooter
          },
  ...
  UNSAFE_componentWillMount() {
    ...
    this.loadData(
      Object.assign({}, params, {
        ...
        pageSize: params.pageSize ? params.pageSize : pagination.pageSize || 10,
      }),
    )
  }

  componentDidUpdate(prevProps) {
    let { params = {}, data, dataUrl } = this.props,
      { pagination } = this.state
    if (
  ...
  loadData = (params, tablePagination) => {
    let { dataUrl, data, dataMethod, dataIndex, onDataLoaded } = this.props,
      { pagination } = this.state
    if (data) {
      this.genRowKey(data)
      ...
      dataPromise.then((results) => {
        ...
        this.setState({
          ...
          pagination: tablePagination
            ? Object.assign({}, tablePagination, { total: results.total })
            : { ...pagination },
        })
  ...
  getColumnSearch = (dataIndex, columnDef) => ({
    ...
    sorter: (a, b) => {
      let aValue = _.get(a, dataIndex, 0),
        bValue = _.get(b, dataIndex, 0)
    ...
    render: (columnData, rowData) => {
      ...
      let text = columnData.value || columnData.value === 0 ? columnData.value : columnData,
      ...
      if ((columnDef.maxWidth || columnDef.width) && !columnDef.render) {
        ...
        return (
          <Tooltip title={tipText || text}>
            <span className={style.textOverflow} style={sto}>
              {text}
            </span>
          </Tooltip>
        )
  ...
  handleColumnHref = (url, rowData) => {
    ...
    if (url.startsWith('http')) {
      ...
      let link = curHref.substring(0, curHref.indexOf('/#/')) + '/#/',
            ...
            render: (value, row, index, params) => {
              ...
              let renderCell = (
                <JSXRender
                  ...
                  jsx={util.renderTemplateString(c.render, {
                    ...extParams,
                    value,
                    row,
                    index,
              ...
              if (c.tooltip) {
                return (
                  <Tooltip
                    title={<JSXRender jsx={util.renderTemplateString(c.tooltip, { row: row })} />}
                  >
