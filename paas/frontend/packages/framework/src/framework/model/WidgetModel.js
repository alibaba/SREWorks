...
export default class WidgetModel extends BaseModel {
  ...
  getCustomItems(widgetMeta) {
    ...
    let configSchema = widgetMeta.configSchema.schema,
      defaults = widgetMeta.configSchema.defaults
    //用旧版本的组件方式暂时只支持json配置
    baseItems.push({
      type: 86,
      name: 'userConfig',
      initValue: { config: Object.assign({}, defaults, this.config || {}) },
      required: false,
      label: '配置',
    })
    /*
  ...
  needReload(nodeParams, nextNodeParams) {
    if (this.dataSourceMeta) {
      ...
      matchVars.forEach((varStr) => {
        ...
        let currValue = _.get(nodeParams, paramKey),
          nextValue = _.get(nextNodeParams, paramKey)
