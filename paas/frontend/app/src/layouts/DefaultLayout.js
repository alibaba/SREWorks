...
class DefaultLayout extends React.Component {
      ...
      if (layout === 'top' && children && children.length > 0) {
        routeItem.push(
          <Route
            path={path}
            key={'_' + path}
            component={() => <ContentWithMenus {...props} routeData={routeData} isV2={isV2} />}
          />,
        )
        return
      } else {
            ...
            if (isV2 && layout === 'custom') {
              ...
              routeItem.push(
                <Route
                  ...
                  path={path}
                  key={'_' + path}
                  component={() => <Redirect to={defaultChildren.path} />}
                />,
              )
            }
          } else {
            if (isV2) {
              routeItem.push(
                <Route
                  ...
                  key={'_' + path}
                  component={() => (
                    <NodeContent {...props} nodeData={routeData} nodeId={routeData.nodeId} />
                  )}
                />,
              )
            } else {
              routeItem.push(
                <Route
                  ...
                  component={() => (
                    <OamContent
                      {...props}
                      menuRouteData={routeData}
                      nodeId={routeData.nodeId}
                      urlRequireNodeId={false}
                      nodeConfig={routeData.config}
                    />
                  )}
                />,
              )
  ...
  render() {
    ...
    const { pathname } = location,
      { productId, nameCn, layout = {}, initAccessUrl } = currentProduct
    //模块未设置默认显示时重定向至模块第一个显示模块,若第一个模块也是树,将会显示404
    if (
      pathname === currentModule.path &&
      !currentModule.component &&
      currentModule.children &&
      currentModule.children.length > 0
    ) {
      let displayChildren = currentModule.children.filter(
          (child) => child.config && !child.config.hidden,
        ),
        defaultPath = ''
      if (displayChildren && displayChildren.length) {
    ...
    if (pathname === `/${productId}` || pathname === '/' || pathname === `/${productId}/`) {
      ...
      currentModule =
        routes.filter((module) => module.config && !module.config.hidden)[0] || currentModule
      let displayChildren = currentModule.children.filter(
          (child) => child.config && !child.config.hidden,
        ),
        defaultPath = ''
    ...
    let hasTop =
        currentModule.layout === 'top-left' &&
        currentModule.children &&
        currentModule.children.length > 0,
      currentRoleId = cacheRepository.getRole(productId),
      currEnvkey = cacheRepository.getBizEnv(productId)
    let currentRole = currentUser.roles.filter((role) => role.roleId === currentRoleId)[0],
      currentEnv = envs.filter((env) => env.value === currEnvkey)[0]
    ...
    return (
      <div className="layout-left">
        <Layout>
          ...
          <Layout
            style={{
              height: `calc(100vh - ${hasTop ? 90 : 48}px)`,
              overflowY: 'scroll',
              overflowX: 'hidden',
            }}
            id="__MAIN_CONTENT__"
