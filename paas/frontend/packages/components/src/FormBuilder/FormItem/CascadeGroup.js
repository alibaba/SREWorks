...
class CascadeGroup extends PureComponent {
  ...
  loadSelectItemsOption = (selectItems, params) => {
      ...
      if (sit.url || sit.data) {
        ...
        if (sit.depend && params) {
          if (Array.isArray(sit.depend)) {
            let { form } = this.props,
              allParamsReady = true,
              dependParams = Object.assign({}, params)
            //依赖项全部有值的情况下进行查询
    ...
    Promise.all(req).then((datas) => {
      ...
      datas.forEach((data, index) => {
        ...
        let dataOptions = data.options || data,
          defalutValue = data.defaultValue || data.initValue
  ...
  fetchRemoteItemOption = (item, searchText) => {
    let { form } = this.props,
      depend = Array.isArray(item.depend) ? item.depend : [item.depend]
  ...
  handleChange = (item, value, option = {}) => {
    ...
    const clearDepend = function (item) {
      selectItems.forEach((sit) => {
        if (
          sit.depend === item.name ||
          (Array.isArray(sit.depend) && sit.depend.includes(item.name))
        ) {
          form.setFieldsValue({
  ...
  render() {
    const { selectItems, loading, fetching } = this.state,
      { model } = this.props
    const { getFieldDecorator } = this.props.form,
      fetchRemoteItemOption = this.fetchRemoteItemOption
    ...
    selectItems.forEach((sit) => {
      ...
      if (sit.searchType === 'remote') {
        ...
        dynamicProps.notFoundContent = fetching ? (
          <Spin size="small" />
        ) : (
          <span>{localeHelper.get('noData', '未找到')}</span>
        )
      }

      selectFormItem.push(
        <div key={sit.name}>
          <FormItem {...formItemLayout} label={label} key={sit.name}>
            {getFieldDecorator(sit.name, {
              initialValue: sit.initValue,
              rules: [
                {
                  required: sit.required,
                  message: sit.label + localeHelper.get('fromItem.required', '必选'),
                },
              ],
            })(
              sit.mode === 'cascader' ? (
                <Cascader
                  options={this.state[[`${sit.name}Options`]]}
                  onChange={(value) => this.handleChange(sit, value)}
                  {...dynamicProps}
                />
              ) : (
                <Select
                  ...
                  {...dynamicProps}
                >
                  {this.state[[`${sit.name}Options`]].map((d) => (
                    <Option optionData={d} value={d.value} key={d.value}>
                      {d.label}
                    </Option>
                  ))}
                </Select>
          ...
          <div style={{ display: 'none' }}>
            {sit.needOption ? (
              <FormItem {...formItemLayout} label={sit.label + 'xxx'} key={sit.name + 'Option'}>
    ...
    return (
      <Spin spinning={loading} key={model.name}>
        {this.inline ? (
          <div style={{ display: 'flex', minWidth: selectFormItem.length * 110 }}>
            {model.label ? (
              <label style={{ width: '25%', marginTop: 8, textAlign: 'end' }}>
                {model.label}&nbsp;:&nbsp;
              </label>
            ) : (
              ''
