...
import { AceViewer } from '@sreworks/components'
...
export default class PageEditor extends React.Component {
    ...
    return (
      <Card style={{ marginLeft: -1, marginTop: -2, height: '100%' }}>
        ...
        <div style={{ marginTop: 24, marginBottom: 24 }}>
          ...
          {current === 2 && <div>收集前面信息进行确认预览展示</div>}
        </div>
        <div>
      ...
      .then((res) => {
        ...
        sunMenuTree =
          (res &&
            res.children &&
            res.children.find(
              (item) => item.nodeTypePath === page_template_meta.parentNodeTypePath,
            )) ||
          {}
        let categoryList = []
        sunMenuTree &&
          sunMenuTree.children.forEach((item) => {
  ...
  render() {
    let {
      ...
      { height = 620, nodeData, contentLoading } = this.props
    let tabEditorContentStyle = { height: height - 42, overflowY: 'auto', overflowX: 'none' },
      { config } = nodeData
    let { pageLayoutType = Constants.PAGE_LAYOUT_TYPE_CUSTOM } = config,
      containerModel = pageModel.getRootWidgetModel()
    return (
            ...
            <div className="feature-bar">
              {activeKey === 'dev' && pageLayoutType === Constants.PAGE_LAYOUT_TYPE_CUSTOM && (
                <Button
                  ...
                  onClick={() => this.handleToolClick(Constants.TOOL_TYPE_ADD)}
                >
                  <div className="wrapper">
                    ...
                    >
                      <path
                        className="path"
                        d="M138.24 158.72v706.56c0 11.264 9.216 20.48 20.48 20.48h299.52v-0.512c10.24-1.024 17.92-9.728 17.92-19.968s-7.68-18.944-17.92-19.968v-0.512H199.68c-11.264 0-20.48-9.216-20.48-20.48V199.68c0-11.264 9.216-20.48 20.48-20.48h624.64c11.264 0 20.48 9.216 20.48 20.48v258.56h0.512c1.024 10.24 9.728 17.92 19.968 17.92s18.944-7.68 19.968-17.92h0.512V158.72c0-11.264-9.216-20.48-20.48-20.48H158.72c-11.264 0-20.48 9.216-20.48 20.48z"
                        p-id="36245"
                      ></path>
                      <path
                        className="path"
                        d="M701.44 307.2H322.56c-11.264 0-20.48-9.216-20.48-20.48s9.216-20.48 20.48-20.48h378.88c11.264 0 20.48 9.216 20.48 20.48s-9.216 20.48-20.48 20.48zM865.792 706.048H506.368c-10.752 0-19.968-8.704-19.968-19.968 0-10.752 8.704-19.968 19.968-19.968h359.936c10.752 0 19.968 8.704 19.968 19.968-0.512 10.752-9.216 19.968-20.48 19.968z"
                        p-id="36246"
                      ></path>
                      <path
                        className="path"
                        d="M666.112 865.792V506.368c0-10.752 8.704-19.968 19.968-19.968 10.752 0 19.968 8.704 19.968 19.968v359.936c0 10.752-8.704 19.968-19.968 19.968-10.752-0.512-19.968-9.216-19.968-20.48z"
                        p-id="36247"
                      ></path>
                    </svg>
              ...
              {saveOrCreat && (
                <Button
                  ...
                  type="primary"
                  onClick={() => this.showTemlateListModal('save')}
                >
                  另存为模板
                </Button>
              )}
              <Button size="small" className="feature-button" onClick={this.handlePreview}>
                <div className="wrapper">
                  ...
                  >
                    <path
                      className="path"
                      d="M886.26 138.29H169.54A60.62 60.62 0 0 0 109 198.84V669.6a60.62 60.62 0 0 0 60.55 60.55h333.62V837.6H136.28v27.29h783.24V837.6H557.75V730.15h328.51a60.62 60.62 0 0 0 60.55-60.55V198.84a60.62 60.62 0 0 0-60.55-60.55z m6 531.31a5.91 5.91 0 0 1-6 6H169.54a5.91 5.91 0 0 1-6-6V198.84a5.91 5.91 0 0 1 6-6h716.72a5.91 5.91 0 0 1 6 6z"
                      p-id="31825"
                    ></path>
                    <path
                      className="path"
                      d="M659.75 416.26L481.62 313.38a21 21 0 0 0-20.92 0.1 20.68 20.68 0 0 0-10.41 18.06v205.53a20.68 20.68 0 0 0 10.41 18.06 21 21 0 0 0 21 0.06l178-102.84a20.85 20.85 0 0 0 0-36.1zM477.58 526V342.56l158.84 91.74z"
                      p-id="31826"
                    ></path>
                  </svg>
          ...
          >
            <div style={tabEditorContentStyle}>
              {pageLayoutType === Constants.PAGE_LAYOUT_TYPE_FLUID && (
                <FluidContentLayoutDesigner
                  showTemlateListModal={this.showTemlateListModal}
            ...
            <Space>
              <Button onClick={this.handleCancel}>取消</Button>
              {
                <Button
                  ...
                  onClick={this.handleCreateFromTemplate}
                  type="primary"
                >
                  确定
                </Button>
        ...
        >
          <section className="template-parent-pane">
            <Tabs
              ...
              tabPosition={'left'}
              onChange={this.handleCategoryClicked}
            >
              {templateList.map((templateCate) => {
                return (
                  <TabPane
                    tab={templateCate.label}
                    style={{ minWidth: 500 }}
                    key={templateCate.serviceType}
                  >
                    <div className="template-pane">
                      {templateCate.children &&
                        templateCate.children.map((item) => {
                          return (
                            <div
                              onClick={() => this.setActive(templateCate, item)}
                              key={item.serviceType}
                              className={
                                item.serviceType === activeTarget
        ...
        >
            ...
            >
              <Select allowClear>
                {categoryList.map((category) => {
                  return (
                    <Option key={category.serviceType} value={category.serviceType}>
                      {category.label}
                    </Option>
                  )
                })}
          ...
          <div style={{ position: 'relative', left: 496, top: -48 }}>
            <Popconfirm
              ...
              title={this.addCategoryForm}
              onConfirm={this.addTemplateCategory}
              okText="确认"
              cancelText="取消"
            >
              <Tooltip title="新增类别">
