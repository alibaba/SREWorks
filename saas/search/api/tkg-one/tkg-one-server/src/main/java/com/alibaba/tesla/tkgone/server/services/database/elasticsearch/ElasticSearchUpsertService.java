...
public class ElasticSearchUpsertService extends ElasticSearchBasic implements InitializingBean {
    ...
    public void insertRelation(List<JSONObject> relation) {
        List<List<JSONObject>> relationList = new ArrayList<>();
        relationList.add(relation);
        insertRelationList(relationList);
    }

    public void insertRelationList(List<List<JSONObject>> relationListList) {

        relationListList = relationListList.parallelStream().map(nodeRelationList -> {
            try {
                return fillRelation(nodeRelationList);
            } catch (Exception e) {
                log.error("预处理关系失败: ", e);
                return null;
            }
        }).collect(Collectors.toList());
        relationListList = deRelations(relationListList);
        List<JSONObject> nodes = relationListList.parallelStream().map(nodeRelationList -> {
            JSONObject nodeJson = new JSONObject();
            nodeJson.put(Constant.INNER_TYPE, Constant.GRAPH_DB_INDEX_NAME);
            nodeJson.put(Constant.INNER_ID, nodeRelationList.get(2).getString(Constant.INNER_TYPE)
                    + Constant.META_PREFIX + nodeRelationList.get(2).getString(Constant.INNER_ID));
            nodeJson.put(Constant.UPSERT_TIME_FIELD, System.currentTimeMillis() / 1000);
            nodeJson.put(Constant.PARTITION_FIELD, Constant.WORLD_START_DATE);
            nodeJson.put("startNodeInfo", nodeRelationList.get(0));
            nodeJson.put("endNodeInfo", nodeRelationList.get(1));
            nodeJson.put("relationInfo", nodeRelationList.get(2));
            return nodeJson;
        }).collect(Collectors.toList());
        upserts("relations", nodes);
    }

    private List<List<JSONObject>> deRelations(List<List<JSONObject>> relations) {
        Set<String> set = new HashSet<>();
        List<List<JSONObject>> deRelations = new ArrayList<>();
        for (List<JSONObject> relation : relations) {
            if (relation == null) {
                continue;
            }
            String id = relation.get(2).getString("__id");
            if (!set.contains(id)) {
                deRelations.add(relation);
                set.add(id);
            }
        }
        return deRelations;
    ...
    private void checkRelation(List<JSONObject> relation) throws Exception {
        if (relation.size() != 3) {
            throw new Exception("关系需要有node-node-relation的三元数组组成");
        }
        for (String metaField : Arrays.asList(Constant.INNER_ID, Constant.INNER_TYPE, Constant.PARTITION_FIELD)) {
            if (!relation.get(0).containsKey(metaField)) {
                throw new Exception(String.format("第一个node没有包含metaField: %s", metaField));
            }
            if (!relation.get(1).containsKey(metaField)) {
                throw new Exception(String.format("第二个node没有包含metaField: %s", metaField));
            }
        }
    }

    private List<JSONObject> fillRelation(List<JSONObject> relation) throws Exception {
        checkRelation(relation);
        String id = relation.get(0).getString(Constant.INNER_ID) + Constant.RELATION_META_SEPARATOR
                + relation.get(1).getString(Constant.INNER_ID);
        String type = relation.get(0).getString(Constant.INNER_TYPE) + Constant.RELATION_META_SEPARATOR
                + relation.get(1).getString(Constant.INNER_TYPE);
        relation.get(2).put(Constant.INNER_ID, id);
        relation.get(2).put(Constant.INNER_TYPE, type);
        relation.get(0).remove(Constant.UPSERT_TIME_FIELD);
        relation.get(1).remove(Constant.UPSERT_TIME_FIELD);
        relation.get(2).remove(Constant.UPSERT_TIME_FIELD);
        return relation;
    }
}
