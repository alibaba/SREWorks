...
const RedirectLink = ({ redirect, icon, label }) => {
  ...
  if (redirect.path.startsWith('http')) {
    return (
      <span onClick={() => window.open(redirect.path)} style={{ color: '#1890fe' }}>
  ...
  if (redirect.type === 'local') {
    ...
    return (
      <span onClick={() => window.open(redirectPath)} style={{ color: '#1890fe' }}>
        {icon ? <LegacyIcon type={icon} /> : null}
        {label}
      </span>
    )
      ...
      <div className="brief-layout-header globalBackground">
        <div className="left-logo">
          <div>
            <span className="logo-link">
              <img
                style={{ width: 24, marginRight: 10, marginTop: -3 }}
                onClick={() => {
                  window.open('/#', '_blank')
                }}
                src={platformLogo}
            ...
            <Divider
              type="vertical"
              style={{ width: 2, height: 20, marginLeft: 14, marginRight: 14 }}
            />
            <span style={{ fontSize: 14, fontWeight: 'bold' }}>{currentProduct.productName}</span>
          </div>
          {showMenu && (
            <div className="menu-container" style={{ minWidth: 200 }}>
        ...
        <div className="right-user" style={{ position: 'relative', display: 'flex' }}>
          {currentProduct.isHasSearch ? (
            <SearchBar changeMenuSate={this.changeMenuSate} global={global}></SearchBar>
          ) : null}
