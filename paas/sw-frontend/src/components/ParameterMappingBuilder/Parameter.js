...
export default class Parameter {
    ...
    getMetaParamDef() {
        ...
        paramMetaDef.push({ type: FormElementType.RADIO, name: 'visible', initValue: this.visible, required: false, label: localeHelper.get('paramMap.isnotView', "是否启用"), tooltip: localeHelper.get('paramMap.billIsnotView', "用户提单页面是否可见"), optionValues: [{ value: 1, label: localeHelper.get('no', '否') }, { value: 2, label: localeHelper.get('is', '是') }] });
