...
class NodeContent extends React.Component {
  ...
  loadContentData = (nodeId) => {
    ...
    let { match, userParams = {}, dispatch, nodeData, currentUser } = this.props,
      nodeParams = {}
    let nodeModel = new NodeModel({ nodeId: nodeData.nodeTypePath })
    //TODO 增加页面数据源的数据获取放入nodeParams
    Promise.all([nodeModel.load(), service.getNodeParams(nodeId)]).then((dataSet) => {
      //由获取的内容数据生成路由
      Object.assign(
        nodeParams,
        dataSet[1],
        { __frontend_route_path: match.path, __currentUser__: currentUser },
        userParams,
        util.getUrlParams(),
      )
  ...
  render() {
    let { loading, pageModel, nodeParams } = this.state,
      { nodeData, routeData } = this.props
    if (loading)
      return (
        <Spin>
    ...
    return (
      <PageContent
        {...this.props}
        initNodeParams={{ ...nodeParams }}
        nodeData={nodeData}
        routeData={routeData}
        pageModel={pageModel}
        pageLayoutType={nodeData.config.pageLayoutType}
      />
    )
  }
}
