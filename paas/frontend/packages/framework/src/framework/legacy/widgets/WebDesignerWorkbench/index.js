...
class TemplateSetting extends React.Component {
  ...
  buildingFormElements = () => {
    ...
    return <div>{[formChildrens[0]]}</div>
  }
...
class WebDesignerWorkbench extends Component {
  ...
  componentWillMount() {
    ...
    let envLabel = 'dev'
    httpClient
      .get(productopsPrefix + `/apps/${appId}/exists?stageId=${envLabel}`, {
        headers: { 'X-Biz-App': util.getNewBizApp() },
      })
      .then((result) => {
        this.setState({
  ...
  createByTemplate = (template) => {
    ...
    let { layout } = this.templateConfig,
      layoutConfig = {}
    if (layout === 'PaaS') {
      layoutConfig = {
        header: {
    ...
    if (properties.devFlag === 'dev') {
      httpClient
        .post('gateway/v2/foundation/frontend-service/frontend/apps/init', blankTemplate, {
          headers: { 'X-Biz-App': util.getNewBizApp(appId) },
        })
        .then((result) => {
          this.setState({
            exists: true,
    ...
    httpClient
      .post('gateway/v2/foundation/appmanager/apps', params, {
    ...
    httpClient
      .post('gateway/v2/common/authProxy/roles', roleParams, {
        headers: { 'X-Biz-App': `${appId},${properties.defaultNamespace},dev` },
      })
      .then((result) => {
        console.log(result)
    ...
    let stageLabel = 'dev'
    return httpClient
      .post(productopsPrefix + '/apps/init?stageId=' + stageLabel, blankTemplate, {
        headers: { 'X-Biz-App': `${appId},${properties.defaultNamespace},dev` },
      })
      .then((result) => {
        this.setState({
  ...
  render() {
    const { __emp_id__ } = this.props.nodeParams,
      { config = {} } = this.props.mode,
      { exists, loading, showCreate, visible } = this.state
    if (loading) {
      return <Spin spinning={loading} />
    }
    // console.log(x_biz_app);
    //menuHolderHeight 前端设计器顶部菜单占用的内容显示区高度，72为前端开发在顶部作为菜单时候的占用高度
    let x_biz_app = `${this.state.appId}|default|dev`,
      { menuHolderHeight = 72 } = config
    //新建的进行模板选择创建
    if (!exists) {
      return (
        <div
                ...
                <div style={{ overflow: 'hidden' }}>
                  <div>
                    <TemplateSettingForm
                      ...
                      onValuesChange={(changedField, allValue) => (this.templateConfig = allValue)}
                    />
                  </div>
                  <div style={{ float: 'right' }}>
                    <Button style={{ marginRight: 10 }} onClick={this.close} size="small">
                      取消
                    </Button>
                    <Button size="small" type="primary" onClick={this.createByTemplate}>
                      确认
                    </Button>
                  </div>
                </div>
    ...
    return (
      <Workbench
        {...this.props}
        stageHeight={$('#__MAIN_CONTENT__').height() - menuHolderHeight}
        x_biz_app={x_biz_app}
        appId={this.state.appId}
        userEmpId={__emp_id__}
        {...config}
      />
    )
  }
