package io.sreworks.common.util;
...
public class Requests {
    ...
    static {
        if (System.getenv("APPMANAGER_ENDPOINT") != null) {
            ...
            String logInfo = "";
            logInfo += " APPMANAGER_ENDPOINT: "+ System.getenv("APPMANAGER_ENDPOINT");
            logInfo += " APPMANAGER_USERNAME: "+ System.getenv("APPMANAGER_USERNAME");
            logInfo += " APPMANAGER_PASSWORD: "+ System.getenv("APPMANAGER_PASSWORD");
            logInfo += " APPMANAGER_CLIENT_ID: "+ System.getenv("APPMANAGER_CLIENT_ID");
            logInfo += " APPMANAGER_CLIENT_SECRET: "+ System.getenv("APPMANAGER_CLIENT_SECRET");
            log.info("CLIENT INIT: " + logInfo);
        } else {
            HTTP_CLIENT = new OkHttpClient.Builder().connectionPool(new ConnectionPool()).build();
            log.info("CLIENT INIT: HTTP_CLIENT");
        }
    }
    ...
    public Requests isSuccessful() throws IOException {
        if (!response.isSuccessful()) {
            throw new IOException(String.format(
                "resp is not successful: %s; retBody: %s", response.toString(), getString()
            ));
        }
