...
/**
 * Component 部署工单 State 处理 Action - TERMINATED
 *
 * @author yaoxing.gyx@alibaba-inc.com
 */
@Slf4j
@Service("TerminatedDeployComponentStateAction")
public class TerminatedDeployComponentStateAction implements DeployComponentStateAction, ApplicationRunner {

    private static final DeployComponentStateEnum STATE = DeployComponentStateEnum.TERMINATED;
    ...
    public void run(ApplicationArguments args) throws Exception {
        timer = meterRegistry.timer("deploy.component.status.terminated.timer");
        counter = meterRegistry.counter("deploy.component.status.terminated.counter");
    ...
    public void run(DeployComponentDO subOrder, Map<String, String> attrMap) {
        ...
        counter.increment();
        log.info("deploy component has reached terminated state|deployAppId={}|deployComponentId={}|cost={}",
        ...
        streamLogService.clean(streamKey, String.format("deploy component has reached terminated state|deployAppId=%s|" +
                "deployComponentId=%s|cost=%s", subOrder.getDeployId(), subOrder.getId(), cost),true);
