...
import { localeHelper, util, cacheRepository, Bus } from '@sreworks/shared-utils'
...
export default class DropDownUser extends React.Component {
  constructor(props) {
    ...
    // 将stageId为空的过滤
    envs = envs.filter(function (item) {
      return item.stageId != ''
    })
  ...
  render() {
    ...
    const UserTitle = () => (
      ...
      >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', paddingLeft: '8px' }}>
          <span style={{ maxWidth: 50, lineHeight: 1.6, paddingTop: 0 }} className="text-overflow">
            {!currentUser
              ? ''
              : currentUser.nickNameCn ||
    ...
    return (
              ...
              <Menu.Item key="switchEnv" style={{ marginBottom: 12 }}>
                <div onClick={(e) => e.stopPropagation()}>
                  <Radio.Group
                    ...
                    buttonStyle="solid"
                    onChange={(e) => this.onEnvChang(e.target.value)}
                  >
                    {envs.map((env) => (
                      <Radio.Button value={env.stageId} key={env.stageId}>
                        {env.stageId}
                      </Radio.Button>
                    ))}
                  </Radio.Group>
      ...
      >
        <a>{this.props.children || <UserTitle />}</a>
      </Dropdown>
    )
  }
}
