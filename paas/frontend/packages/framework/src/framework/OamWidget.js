...
export default class OamWidget extends React.Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    if (widget.elementId) {
      oamTreeService
        .getWidgetMeta(nodeId || nodeParams.__nodeId__, widget.elementId, parameters)
        .then((widgetDef) => {
          this.setState({
  ...
  loadData = (widgetDataSource, mode) => {
    if (
      widgetDataSource.type &&
      widgetDataSource.type === 'API' &&
      Constants.EXCLUDE_COMP_TYPES.includes(mode.type)
    ) {
      return false
    }
    let { nodeParams = {}, nodeData = {} } = this.props,
      urlParams = util.getUrlParams()
    let paramsSet = Object.assign({}, nodeParams, urlParams),
      { dependParams } = mode.config
    ...
    let { api, params = {}, method = 'get' } = widgetDataSource,
      req = null
    if (api && api.includes('$(')) {
  ...
  getMode = (modeTemplate, props) => {
    ...
    let { nodeParams, widgetParams = {} } = props,
      urlParams = util.getUrlParams()
    ...
    let cloneTemplate = JSON.parse(JSON.stringify(modeTemplate)),
      children
  ...
  render() {
    let { widget, widgetParams, ...contentProps } = this.props,
      { mode, widgetData } = this.state
    ...
    let { align, display, fill } = mode.config || {},
      dynamicStype = mode.config.cStyle || {}
    ...
    //组件统一增加默认值设定
    let widgetDefaultParams = Object.assign(
      {},
      _.get(contentProps, 'tabMate.config.defaultParams'),
      _.get(mode, 'config.defaultParams'),
    )
    //增加流程组件回调
    ...
    return (
      <div
        style={dynamicStype}
        className={
          dynamicStype.fill || fill
            ? 'globalBackground abm-widget-container'
        ...
        <ErrorBoundary>
          {WidgetFactory.createWidget(mode, {
            ...
            widgetData: widgetData
              ? _.isString(widgetData)
                ? widgetData
                : Object.assign({}, widgetParams, widgetData)
              : contentProps.widgetData || false,
            auths: mode.defAuths || [],
          })}
        </ErrorBoundary>
      </div>
