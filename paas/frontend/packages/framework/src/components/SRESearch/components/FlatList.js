          ...
          <div className="flat-list">
            {suggestList.map((item, index) => {
              return (
                <div
                  key={index}
                  className="suggest-list-cell"
                  onClick={() => this.turnToNewPath(item.url)}
                  style={{ height: '30px', paddingRight: 20 }}
                >
                  <span>{item.type}</span>
            ...
            {suggestList.length > 49 && (
              <div onClick={this.searchMore} className="more-btn">
                <MoreOutlined />
                查看更多
              </div>
            )}
