...
import './index.less'
import { FormElementFactory, AceViewer } from '@sreworks/components'
...
class DataSource extends Component {
    ...
    return (
      <div>
        <Form.Item {...formItemLayout} label={label}>
          ...
          {visibleMap[dataIndex] && (
            <Form.Item>
              {getFieldDecorator(dataIndex, {
                initialValue:
                  dataIndex === 'JSON' && typeof value[dataIndex] === 'object'
                    ? JSON.stringify(value[dataIndex], null, 2)
                    : value[dataIndex],
              })(dom)}
            </Form.Item>
          )}
  ...
  renderFormItem = (type) => {
    console.log(type)
    let { options = {}, source = '' } = tempDS.expand_service_type || {},
      mapping = []
    const { getFieldDecorator } = this.props.form
    ...
    return (
      <div>
        ...
        {[json, nodeParams].indexOf(type) !== -1 &&
          this.hiddenDom(
            <AceEditor
              mode="json"
              style={{ height: 400, width: 700 }}
            ...
            <Form.Item label="节点实体" {...formItemLayout}>
              {getFieldDecorator('entityName', {
                ...
                <Select placeholder="选择节点实体" showSearch onSelect={this.handleTableChange}>
                  {tables.map((t) => {
                    return (
                      <Option value={t.tableName} key={t.tableName}>
                        {t.alias}({t.tableName})
                      </Option>
                    )
                  })}
            ...
            <Form.Item label="节点名称" {...formItemLayout}>
              {getFieldDecorator('name', {
                ...
                <Select placeholder="请选择列" showSearch>
                  {columns.map((c) => {
                    return (
                      <Option value={c.columnName} key={c.columnName}>
              ...
              return (
                <Row key={filter.key} type="flex" justify="center">
                  ...
                  <Col span={1}>
                    <MinusCircleOutlined
                      style={{ marginTop: 12 }}
                      onClick={() => this.removeFilter(filter.key)}
                    />
                  </Col>
                </Row>
              )
  ...
  render() {
    ...
    if (description && formats) {
      dataComponent = (
        <Card title="数据源返回数据结构" size={'small'}>
          ...
          {formats.map((format) => {
            return (
              <Card style={{ marginTop: 6 }} size={'small'} type="inner" title={format.description}>
                <AceViewer
                  model={{
                    showDiff: false,
    ...
    return (
      <Row>
        ...
        <Col span={Object.keys(dataFormatter).length > 0 ? 8 : 0}>{dataComponent}</Col>
      </Row>
    )
  }
}
...
export default Form.create({
  onValuesChange: (props, changedValues, allValues) => {
    ...
    props.onValuesChange &&
      props.onValuesChange(
        changedValues,
        allValues.type ? Object.assign({}, props.value, allValues) : null,
      )
  },
})(DataSource)
