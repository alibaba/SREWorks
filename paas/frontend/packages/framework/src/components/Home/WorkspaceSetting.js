...
export default class WorkspaceSetting extends React.Component {
  ...
  handleAddWorkspace = () => {
    const { dispatch, home } = this.props
    dispatch({
      type: 'home/addWorkspace',
      workspace: { name: '新建桌面 ' + home.workspaces.length },
    })
  }
  ...
  handleSave = () => {
    ...
    AppService.postWorkspaces(params, namespaceId, stageId).then((res) => {})
  }
  ...
  onSubmit = (res) => {
    ...
    if (res) {
      customQuickList.push({
        id: uuidv4(),
        appName: res.appName,
        href: res.href,
        introduction: res.href,
      })
  ...
  render() {
    const { visible, settingType, quickVisible, isDelete } = this.state,
      { home } = this.props
    ...
    const footer = () => {
      if (settingType === 'add') {
        return [
          <Button type="danger" onClick={this.handleDeleteQuick}>
            删除快捷方式
          </Button>,
          <Button type="primary" onClick={this.handleCreateQuick}>
            新建快捷方式
          </Button>,
          <Button type="primary" onClick={() => window.open(`/#/swadmin/mart/local`)}>
            去部署
          </Button>,
    ...
    return (
      <div className="workspace-setting">
        ...
        <Modal title="新建快捷方式" visible={quickVisible} footer={null} onCancel={this.onClose}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            ...
            <Button
              style={{ marginLeft: 16 }}
              type="primary"
              onClick={() => this.onSubmit.validate()}
            >
              确定
            </Button>
          </div>
        ...
        <Modal
          title="设置桌面背景"
          width="60%"
          visible={bacgroundSettingVisible}
          footer={null}
          onCancel={this.onCloseBackgroundSetting}
        >
          <div className="imglist-panel">
            {!!mixImglist.length &&
              mixImglist.map((img, j) => {
                return (
                  <div
                    className={
                      activeBackgroundImgIndex === j ? 'img-wrapper isLight' : 'img-wrapper'
                    }
                    key={img}
                  >
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            ...
            <Button
              style={{ marginLeft: 16 }}
              type="primary"
              onClick={() => this.handleUpdateWorkspace()}
            >
              确定
            </Button>
          </div>
                  ...
                  {workspaces.map((child, i) => {
                    return (
                      <div className="desktop-item-block">
                        {i > 0 && (
                          <span
                            className="removeHandle"
                            onClick={() => this.handleRemoveWorkspace(i)}
                          >
                            <DeleteOutlined />
                          </span>
                        )}
                        <div>
                          <div className="icon-wrapper">
                            ...
                            {originMixImglist.includes(child.background) && (
                              <img
                                className="icon"
                                src={
                                  localImglist.includes(child.background)
                                    ? requiredImglist[localImglist.indexOf(child.background)]
                                    : child.background
                                }
                                onClick={() => this.showSettingModal(i)}
                              />
                            )}
                            {!originMixImglist.includes(child.background) && (
                              <img
                                className="icon"
                                src={requiredImglist[0]}
                                onClick={() => this.showSettingModal(i)}
                              />
                            )}
                            <div class="icon-mask" onClick={() => this.showSettingModal(i)}>
                              <div
                                style={{
              ...
              <TabPane tab="桌面背景" key="background">
                <Spin spinning={loading}>
                  <div className="appSet">
                    {mixImglist &&
                      !!mixImglist.length &&
                      mixImglist.map((child, i) => {
                        return (
                          <div key={child} className="desktop-item-block">
                            {i > 3 && (
                              <span
                                className="removeHandle"
                                onClick={() => this.deleteImage(child)}
                              >
                                <DeleteOutlined />
                              </span>
                            )}
