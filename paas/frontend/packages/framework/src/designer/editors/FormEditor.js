...
class FormEditor extends PureComponent {
  ...
  remove = (targetKey) => {
    ...
    this.setState({
      ...
      activeKey: (newParameters[0] && newParameters[0].onlyKey) || '',
    })
  }
  ...
  render() {
    let { parameters, activeKey } = this.state,
      panes = [],
      { title, tabPosition } = this.props
        ...
        <TabPane tab={parameter.label || parameter.name} key={parameter.onlyKey} closable={true}>
          <FormItemEditor
            ...
            onValuesChange={(changedValues, allValues) =>
              this.handleParameterChanged(parameter, allValues)
            }
          />
        </TabPane>,
      )
          ...
          tabBarExtraContent={
            <div>
              <a onClick={() => this.onEdit(null, 'add')} style={{ marginRight: 12, fontSize: 16 }}>
                <PlusOutlined />
                <span style={{ fontSize: 14 }}>添加</span>
              </a>
            </div>
          }
          type="editable-card"
