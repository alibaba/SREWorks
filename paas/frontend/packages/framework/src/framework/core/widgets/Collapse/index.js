...
export default class TabsRender extends Component {
  render() {
    ...
    return (
      <Collapse accordion={accordion} bordered={bordered} expandIconPosition={expandIconPosition}>
        {(widgetConfig.panes || []).map((item) => {
          ...
          return (
            <Panel showArrow={showArrow} header={item.title} key={item.key} disabled={disabled}>
