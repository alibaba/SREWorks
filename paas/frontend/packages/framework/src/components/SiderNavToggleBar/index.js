...
export default class Desktop extends React.Component {
  ...
  handleClick = (e, item) => {
    if (item.appId) {
      window.open(
        '#/' +
          item.appId +
          `?namespaceId=${item.namespaceId || properties.defaultNamespace}&stageId=${
            item.stageId || properties.defaultStageId
          }`,
      )
    } else if (item.href) {
  ...
  render() {
    ...
    return (
      <div
        className="abm-toggle-menu-bar"
        style={theme === 'light' ? { background: '#fffffd' } : { background: '#2c2f40' }}
      >
        <div onClick={this.switchNavVisible}>
          <LegacyIcon
            type={this.props.icon || 'menu'}
            style={
              this.props.cStyle || {
                height: 48,
                lineHeight: '25px',
        ...
        <Drawer
          ...
          bodyStyle={
            this.props.theme !== 'dark'
              ? { top: 0, padding: 0 }
              : { background: '#001529', color: '#fff', top: 0, padding: 0 }
          }
        >
            ...
            >
              ...
              >
                <div
                  ...
                  className={
                    this.props.theme === 'dark'
                      ? 'abm-toggle-menu-bar-product-nav-content abm-toggle-menu-bar-product-nav-content-dark'
                      : 'abm-toggle-menu-bar-product-nav-content abm-toggle-menu-bar-product-nav-content-light'
                  }
                >
                  <NavSelectPanel />
                </div>
              ...
              {collectList.map((item, i) => {
                ...
                return (
                  <Menu.Item key={item.appId || item.id}>
                    <div
                      id={'menu_' + (item.appId || item.id)}
