...
export default class PageContent extends React.PureComponent {
  ...
  render() {
    let { pageModel, pageLayoutType = Constants.PAGE_LAYOUT_TYPE_CUSTOM } = this.props,
      { pageParamsLoaded, currentAction } = this.state
    if (pageParamsLoaded) {
      return <Spin className="globalSpin" />
    }
    let widgets = pageModel.getWidgets(),
      props = this.props
      ...
      <div>
                ...
                <div className="PAGE_LAYOUT_TYPE_CUSTOM">
                  {
                    ...
                    >
                      {widgets.map((widget) => {
                        return (
                          <div
                            key={widget.uniqueKey || widget.config.uniqueKey || 'unknown'}
                            data-grid={
                              widget.gridPos || widget.config.gridPos || Constants.UNKNOWN_GRID_POS
                            }
                          >
                            <WidgetCard
                              {...props}
        ...
        {currentAction && (
          <ActionPanel
            ...
            nodeModel={pageModel.getNodeModel()}
