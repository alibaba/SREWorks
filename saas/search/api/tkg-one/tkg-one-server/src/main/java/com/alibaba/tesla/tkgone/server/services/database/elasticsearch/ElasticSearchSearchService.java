...
public class ElasticSearchSearchService extends ElasticSearchBasic {
    ...
    public JSONObject getTypeIdRelationNodeMetaInfo(String type, String id) throws Exception {
        JSONObject queryJson = new JSONObject();
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_TYPE), type);
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_ID), id);
        JSONObject metaJson = new JSONObject();
        metaJson.put("aggField", String.format("endNodeInfo.%s", Constant.INNER_TYPE));
        metaJson.put("aggSize", 10000);
        metaJson.put("type", Constant.GRAPH_DB_INDEX_NAME);
        JSONObject requestJson = new JSONObject();
        requestJson.put("query", queryJson);
        requestJson.put("meta", metaJson);
        JSONObject retJson = new JSONObject();
        for (JSONArray jsonArray : aggByKv(requestJson).toJavaList(JSONArray.class)) {
            retJson.put(jsonArray.getString(0), jsonArray.getString(1));
        }
        return retJson;
    }

    public JSONArray getRelationNodes(String type, String id, String relationType, int from, int size)
            throws Exception {
        JSONObject queryJson = new JSONObject();
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_TYPE), type);
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_ID), id);
        JSONObject metaJson = new JSONObject();
        metaJson.put("from", from);
        metaJson.put("size", size);
        metaJson.put("type", Constant.GRAPH_DB_INDEX_NAME);
        JSONArray sortJson = new JSONArray();
        sortJson.add(Constant.INNER_ID);
        JSONObject requestJson = new JSONObject();
        requestJson.put("query", queryJson);
        requestJson.put("meta", metaJson);
        requestJson.put("sort", sortJson);
        return queryByKv(requestJson);
    }

    public JSONObject listRelationType() throws Exception {
        JSONObject metaJson = new JSONObject();
        metaJson.put("aggField", String.format("relationInfo.%s", Constant.INNER_TYPE));
        metaJson.put("aggSize", 10000);
        metaJson.put("type", Constant.GRAPH_DB_INDEX_NAME);
        JSONObject requestJson = new JSONObject();
        requestJson.put("meta", metaJson);
        JSONObject ret = new JSONObject();
        for (JSONArray jsonArray : aggByKv(requestJson).toJavaList(JSONArray.class)) {
            ret.put(jsonArray.getString(0), jsonArray.get(1));
        }
        return ret;
    }

    public void deleteRelationType(String relationType) throws Exception {
        JSONObject queryJson = new JSONObject();
        queryJson.put(String.format("relationInfo.%s", Constant.INNER_TYPE), relationType);
        elasticSearchDeleteByQueryService.deleteByKvFull(Constant.GRAPH_DB_INDEX_NAME, false, queryJson);
    }

    public JSONArray getRelationNodes(String startNodeType, String startNodeId, String endNodeType, String endNodeId,
            String relationType, String relationId, Integer limit) throws Exception {
        JSONObject queryJson = new JSONObject();
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_TYPE), startNodeType);
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_ID), startNodeId);
        queryJson.put(String.format("endNodeInfo.%s", Constant.INNER_TYPE), endNodeType);
        queryJson.put(String.format("endNodeInfo.%s", Constant.INNER_ID), endNodeId);
        queryJson.put(String.format("relationInfo.%s", Constant.INNER_TYPE), relationType);
        queryJson.put(String.format("relationInfo.%s", Constant.INNER_ID), relationId);
        List<String> keys = new ArrayList<>(queryJson.keySet());
        for (String key : keys) {
            String value = queryJson.getString(key);
            if (StringUtils.isEmpty(value)) {
                queryJson.remove(key);
            }
        }
        JSONObject metaJson = new JSONObject();
        metaJson.put("type", Constant.GRAPH_DB_INDEX_NAME);
        metaJson.put("size", limit);
        JSONArray sortJson = new JSONArray();
        sortJson.add(Constant.INNER_ID);
        JSONObject requestJson = new JSONObject();
        requestJson.put("query", queryJson);
        requestJson.put("meta", metaJson);
        requestJson.put("sort", sortJson);
        return queryByKv(requestJson).toJavaList(JSONObject.class).parallelStream()
                .map(node -> node.getJSONObject("endNodeInfo")).collect(Collectors.toCollection(JSONArray::new));
    }

    public void delete(String nodeType, String startNodePartition, String startNodeType, String startNodeId,
            String endNodeType, String endNodeId, String relationType, String relationId) throws Exception {
        JSONObject queryJson = new JSONObject();
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_TYPE), startNodeType);
        queryJson.put(String.format("startNodeInfo.%s", Constant.INNER_ID), startNodeId);
        queryJson.put(String.format("endNodeInfo.%s", Constant.INNER_TYPE), endNodeType);
