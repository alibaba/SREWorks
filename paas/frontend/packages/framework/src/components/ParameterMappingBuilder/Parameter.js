...
export default class Parameter {
  constructor(paramData, runtimeContext) {
    ...
    this.defModel =
      (typeof paramData.defModel === 'string'
        ? JSON.parse(paramData.defModel)
        : paramData.defModel) || {}
    //参数次序
  ...
  getMetaParamDef() {
    ...
    paramMetaDef.push({
      ...
      initValue: this.name,
      required: true,
      label: localeHelper.get('paramMap.paramName', '参数名称'),
      tooltip: localeHelper.get('paramMap.paramNameRule', '参数名,需为英文'),
    })
    paramMetaDef.push({
      type: FormElementType.INPUT,
      name: 'label',
      initValue: this.label,
      required: false,
      label: localeHelper.get('paramMap.paramTitle', '参数标题'),
      tooltip: localeHelper.get('paramMap.paramShowName', '参数在表单页面显示的标签名'),
    })
    paramMetaDef.push({
      ...
      initValue: this.type,
      required: true,
      label: localeHelper.get('paramMap.inputType', '输入方式'),
      optionValues: FormElementType.SIMPLE_TYPE_LABEL_MAPPING_OPTIONS,
      tooltip: localeHelper.get('paramMap.enterShowType', '用户录入参数的显示方式'),
    })
    paramMetaDef.push({
      type: FormElementType.RADIO,
      name: 'required',
      initValue: this.required,
      required: false,
      label: localeHelper.get('paramMap.isnotMustFill', '是否必填'),
      optionValues: [
        { value: 1, label: localeHelper.get('no', '否') },
        { value: 2, label: localeHelper.get('is', '是') },
      ],
    })
    paramMetaDef.push({
      type: FormElementType.RADIO,
      name: 'visible',
      initValue: this.visible,
      required: false,
      label: localeHelper.get('paramMap.isnotView', '是否启用'),
      tooltip: localeHelper.get('paramMap.billIsnotView', '用户提单页面是否启用'),
      optionValues: [
        { value: 1, label: localeHelper.get('no', '否') },
        { value: 2, label: localeHelper.get('is', '是') },
      ],
    ...
    paramMetaDef.push({
      type: FormElementType.RADIO,
      name: 'readonly',
      initValue: this.readonly,
      required: false,
      label: localeHelper.get('paramMap.isnotOnlyRead', '是否只读'),
      tooltip: localeHelper.get('paramMap.billIsnotEdit', '用户提单页面是不可修改'),
      optionValues: [
        { value: 1, label: localeHelper.get('no', '否') },
        { value: 2, label: localeHelper.get('is', '是') },
      ],
    ...
    paramMetaDef.push({
      type: FormElementType.RADIO,
      name: 'validateType',
      initValue: this.validateType,
      required: true,
      label: localeHelper.get('paramMap.verificationType', '校验类型'),
      optionValues: [
    ...
    if (this.type === FormElementType.MACHINE_TARGET) {
      ...
      paramMetaDef.push({
        type: FormElementType.SELECT_TAG,
        name: 'defaultValue',
        initValue: this.defaultValue.length > 0 ? this.defaultValue : this.value,
        required: false,
        label: localeHelper.get('paramMap.defaultValue', '默认值'),
        inputTip: localeHelper.get('paramMap.inputDefaultValue', '直接输入默认值'),
      })
    }
    paramMetaDef.push({
      type: FormElementType.SELECT_TAG,
      name: 'validValue',
      initValue: this.validValue,
      required: false,
      label: localeHelper.get('paramMap.effectiveValue', '有效值'),
      inputTip: localeHelper.get('paramMap.selectMsg', '定义单选/多选形式的可选值,可直接输入选项'),
    })
    paramMetaDef.push({
      type: FormElementType.INPUT,
      name: 'toolTip',
      initValue: this.toolTip,
      required: false,
      label: localeHelper.get('paramMap.paramHint', '参数提示'),
    })
    paramMetaDef.push({
      type: FormElementType.DISABLED_INPUT,
      name: 'order',
      initValue: this.order,
      required: false,
      label: localeHelper.get('paramMap.paramSerial', '参数序号'),
      validateType: 'number',
      inputTip: localeHelper.get(
        'paramMap.paramSerialMsg',
        '数字类型,定义此参数在表单中的顺序位置,不存在时按照tab页顺序',
      ),
    })
    paramMetaDef.push({
      type: FormElementType.INPUT,
      name: 'validateReg',
      initValue: this.validateReg,
      required: false,
      label: '校验正则',
      inputTip: '合法值的正则表达式',
    })
    paramMetaDef.push({
      type: FormElementType.INPUT,
      name: 'initApi',
      initValue: this.initApi,
      required: false,
      label: '参数接口',
      inputTip: '定义参数的初始值和下拉值的接口地址,返回格式{initValue:xxx,options:[...]}',
    })
    paramMetaDef.push({
      type: FormElementType.INPUT,
      name: 'visibleExp',
      initValue: this.visibleExp,
      required: false,
      label: '可见表达式',
      inputTip: '直接用formValues.参数名来引用表单其他参数值',
    })
    paramMetaDef.push({
      type: 86,
      name: 'defModel',
      initValue: this.defModel,
      required: false,
      label: '参数JSON配置',
      height: 200,
      showDiff: false,
    ...
    paramMetaDef.push({
      type: FormElementType.RADIO,
      name: 'isHidden',
      initValue: this.isHidden,
      required: false,
      label: localeHelper.get('paramMap.isnotView', '是否可见'),
      tooltip: localeHelper.get('paramMap.billIsnotView', '用户提单页面是否可见'),
      optionValues: [
        { value: 1, label: localeHelper.get('no', '否') },
        { value: 2, label: localeHelper.get('is', '是') },
      ],
  ...
  getInstanceParamDef() {
    ...
    paramInstanceDef.push({
      ...
      initValue: this.type,
      required: true,
      label: localeHelper.get('paramMap.inputType', '参数类型'),
      optionValues: [
        { value: 1, label: '文本' },
        { value: FormElementType.MACHINE_TARGET, label: '机器选择' },
      ],
      tooltip: localeHelper.get('paramMap.enterShowType', '用户录入参数的显示方式'),
    })
    if (this.type === FormElementType.MACHINE_TARGET) {
      ...
      paramInstanceDef.push({
        ...
        initValue: this.defaultValue.length > 0 ? this.defaultValue : this.value,
        required: true,
        label: localeHelper.get('paramMap.defaultValue', '参数值'),
        inputTip: localeHelper.get('paramMap.inputDefaultValue', '直接输入参数值'),
      })
    }
  ...
  getSyncParamWidgetDef() {
    ...
    if (this.type === FormElementType.INPUT || this.type === FormElementType.TEXTAREA) {
      //普通输入类
      widgetDef.initValue = Array.isArray(initValue) ? initValue.join(',') : initValue
    } else if (
      this.type === FormElementType.GRID_CHECKBOX ||
      this.type === FormElementType.SELECT ||
      this.type === FormElementType.MULTI_SELECT ||
      this.type === FormElementType.RADIO ||
      this.type === FormElementType.CHECKBOX ||
      this.type === FormElementType.SELECT_TAG ||
      this.type === 14 ||
      this.type === 17 ||
      this.type === 87
    ) {
      ...
      if (
        ...
        typeof widgetDef.initValue === 'string' &&
        isFinite(widgetDef.initValue)
      ) {
      ...
      if (defaultTime) {
        let { stimeOffsetFromNow, etimeOffsetFromNow } = defaultTime,
          stime = moment().subtract(2, 'h'),
          etime = moment()
      ...
      widgetDef.initValue = initValue
    } else if (this.type === FormElementType.SELECT_TREE || this.type === 12) {
      //选择树
  ...
  loadInitData() {
    let { transform, optionMapping, remote } = this.defModel,
      loader = false
    //增加数据源支持
  ...
  updateValue(value) {
    ...
    if (this.type === FormElementType.DATE) {
      ...
      this.type === FormElementType.INPUT ||
      this.type === FormElementType.TEXTAREA ||
      this.type === FormElementType.HIDDEN_INPUT
    ) {
  ...
  getValue() {
    ...
    if (
      (this.type === FormElementType.INPUT || this.type === FormElementType.TEXTAREA) &&
