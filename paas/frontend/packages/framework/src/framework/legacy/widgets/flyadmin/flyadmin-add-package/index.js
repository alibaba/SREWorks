...
class FlyAdminAddPackage extends React.Component {
  ...
  getApplicationServiceList = () => {
    ...
    flyAdminService.getApplicationServiceList(params).then((data) => {
      data.map((item) => {
  ...
  getApplicationVersion = () => {
    ...
    flyAdminService.getApplicationVersion(params).then((data) => {
        ...
        this.setState(
          {
            ...
            k8SMicroserviceList,
            jobList,
            allSelectList: microserviceList
              .concat(resourceAddonList)
              .concat(internalAddonList)
              .concat(k8SMicroserviceList)
              .concat(jobList),
            packageData: { ...values },
            current: current + 1,
          },
  ...
  getDataInputList = (item, index, type) => {
    ...
    flyAdminService.getMicroservice(params).then((res) => {
      res.envList.map((evn) => {
        evn.valueType = 'select'
      })
      list[index].branch =
        res.containerObjectList && res.containerObjectList[0] && res.containerObjectList[0].branch
  ...
  getListComponentVersions = (type, index, res) => {
    ...
    flyAdminService.getListComponentVersions(params).then((data) => {
        ...
        k8s.envList.map((env) => {
          if (env.name === record.name) {
            if (field === 'componentName') {
              ...
              flyAdminService.getListDataOutputs(params).then((data) => {
  ...
  formatK8SMicroServiceSelect = (item, index, type) => {
    ...
    return (
      <div>
        {/*选择k8s微应用大版本*/}
        <Row type="flex" align="middle" style={{ fontWeight: 500 }}>
          {index + 1}、{item.componentLabel}
        </Row>
        <Row type="flex" align="middle" className="env-title">
          ...
          <Col span={20}>
            <Input
              ...
              placeholder="输入代码分支"
              onChange={(res) =>
                this.handleChangeK8SMicroservice(
                  res.target && res.target.value,
                  index,
                  'branch',
                  type,
                )
        ...
        <Row>
          <Col span={14} offset={4}>
            <a className="trigger" onClick={() => this.handleExpand(item.componentName)}>
              {expand[item.componentName]
                ? localeHelper.get('common.close', '收起')
                : localeHelper.get('common.open', '展开')}
              <LegacyIcon type={expand[item.componentName] ? 'up' : 'down'} />
            </a>
          </Col>
        </Row>
        {expand[item.componentName] && (
            ...
            <Row type="flex" align="middle" className="env-title">
              ...
              <Col span={20}>
                <Select
                  ...
                  placeholder={'请选择依赖服务'}
                  onChange={(res) =>
                    this.handleChangeK8SMicroservice(res, index, 'dependComponentList', type)
                  }
                >
                  {allSelectList
                    .filter((k8s) => k8s.componentName !== item.componentName)
                    .map((option) => {
                      ...
                      render: (text, record, index) => {
                        return (
                          <Select
                            defaultValue="select"
                            onChange={(res) =>
                              this.onChangeMicroService(res, item, record, 'valueType', type)
                            }
                          >
                            <Option value="select">选择</Option>
                            <Option value="input">输入</Option>
                      ...
                      render: (text, record) => {
                        return record.valueType === 'input' ? (
                          ...
                          <Select
                            ...
                            placeholder="请选择依赖组件"
                            onChange={(res) =>
                              this.onChangeMicroService(res, item, record, 'componentName', type)
                            }
                          >
                            {resourceAddonList.map((option) => {
                              return (
                                <Option value={option.componentName}>
                      ...
                      render: (text, record) => {
                        return record.valueType === 'input' ? (
                          <Input
                            ...
                            value={record.paramDefaultValue}
                            placeholder={'请输入输出字段'}
                          />
                        ) : (
                          <Select
                            onChange={(res) =>
                              this.onChangeMicroService(res, item, record, 'dataOutputName', type)
                            }
                            value={record.dataOutputName || undefined}
                            placeholder={'请选择输出字段'}
                          >
                            {record &&
                              (record.dataOutputs || []).map((option) => {
        ...
        <Col span={21} offset={3}>
          ...
          <Collapse defaultActiveKey={getFieldValue('packageTypes')} onChange={this.onChange}>
            ...
            <Panel header="资源 Addon" key="RESOURCE_ADDON">
              <div>已选择的资源 Addon</div>
              <div>
                {!!resourceAddonList.length ? (
                  resourceAddonList.map((item, index) => {
                    return (
            ...
            <Panel header="内置服务组件" key="INTERNAL_ADDON">
              <div>已选择的内置 Addon</div>
              <div>
                {!!internalAddonList.length ? (
                  internalAddonList.map((item) => {
                    return (
                      <Row type="flex" align="middle">
                        {item.componentLabel}
  ...
  render() {
    ...
    return (
      <Spin spinning={loading}>
        <div className="flyadmin-package">
            ...
            <div style={{ marginTop: 20 }}>
              {current === 0 && (
                <div>
                  ...
                  {!!getFieldValue('packageTypes').length && (
                    <Row style={{ marginBottom: 10 }}>
                      <Col span={18} offset={6}>
                        <Collapse
                          defaultActiveKey={getFieldValue('packageTypes')}
                            ...
                            <Panel header="微服务" key="MICROSERVICE">
                              ...
                              <Table
                                ...
                                rowSelection={{
                                  ...
                                  onChange: (res) =>
                                    this.onSelectMicroServiceChange(
                                      res,
                                      'selectedMicroServiceRowKeys',
                                    ),
                                }}
                                columns={columns}
                                dataSource={serviceMap['MICROSERVICE'].concat(
                                  serviceMap['K8S_MICROSERVICE'],
                                )}
                              />
                            </Panel>
                          )}
                            ...
                            <Panel header="Job" key="K8S_JOB">
                              <Table
                                ...
                                rowSelection={{
                                  ...
                                  onChange: (res) =>
                                    this.onSelectMicroServiceChange(res, 'selectedJobRowKeys'),
                                }}
                                rowKey="componentName"
                                columns={columns}
              ...
              {current === 2 && (
                <div style={{ textAlign: 'center' }}>
                  ...
                  <div>
                    <Button
                      type="primary"
                      onClick={() => this.props.closeAction && this.props.closeAction(true)}
                    >
                      返回列表
                    </Button>
                  </div>
