...
class FilterSetting extends React.Component {
  constructor(props) {
    ...
    paramsDef.push({
      ...
      name: 'outputName',
      initValue: outputName,
      required: false,
      label: '输出对象名称',
      tooltip: '过滤器表单值输出到填写的参数名对象中,为空时表单项值全部独立输出到节点参数域中',
    })
    ...
    paramsDef.push({
      ...
      name: 'enterSubmit',
      initValue: enterSubmit === true || false,
      required: false,
      label: 'Enter触发查询',
      optionValues: [
        { value: true, label: '是' },
    ...
    paramsDef.push({
      type: FormElementType.RADIO,
      name: 'autoSubmit',
      initValue: autoSubmit === true || false,
      required: false,
      label: '值改变触发查询',
      optionValues: [
        { value: true, label: '是' },
    ...
    paramsDef.push({
      type: FormElementType.RADIO,
      name: 'showSearch',
      initValue: showSearch === true || false,
      required: false,
      label: '显示查询按钮',
      optionValues: [
        { value: true, label: '是' },
    ...
    paramsDef.push({
      ...
      tooltip:
        '预先设定好过滤器的过滤值,进行快捷查询。数组的每个元素包含 values、title、category(可选用于分类)三个key，values为过滤器中的表单项值,title为过滤项名称,category启用场景分组',
    })
    paramsDef.push({
      type: FormElementType.INPUT,
      name: 'visibleExpression',
      initValue: visibleExpression,
      required: false,
      label: '可见表达式',
      tooltip: '通过表达式条件来实现对过滤器区块进行隐藏/展示',
      inputTip: '直接用nodeParams.参数名或$(参数名)来引用参数阈其他参数值',
    })
    if (this.isTabFilter) {
      paramsDef = [
        {
        ...
        },
        {
          type: FormElementType.RADIO,
          name: 'tabPosition',
          initValue: tabPosition,
          required: false,
          label: 'tab展示位置',
          optionValues: [
            { value: 'top-left', label: '左上' },
            { value: 'top-right', label: '右上' },
            { value: 'bottom-left', label: '左' },
          ],
        ...
        },
        {
          ...
          required: false,
          label: '可见表达式',
          tooltip: '直接用nodeParams.参数名或$(参数名)来引用参数阈其他参数值',
          inputTip: '直接用nodeParams.参数名或$(参数名)来引用参数阈其他参数值',
        },
      ]
    }
    ...
    if (this.isFilterForm) {
      return (
        <Row>
          ...
          <Col span={9}>{[formChildrens[6], formChildrens[8], formChildrens[7]]}</Col>
        </Row>
      )
    }
    return (
      <Row>
        ...
        <Col span={9}>{[formChildrens[6], formChildrens[5]]}</Col>
      </Row>
    )
  }
