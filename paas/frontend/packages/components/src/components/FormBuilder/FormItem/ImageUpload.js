...
export default class ImageUpload extends Component {
  ...
  render() {
    const { fileList, loading, imageUrl, imgList } = this.state,
      { onChange, model, isMulti = false } = this.props
    ...
    return (
      <div>
        ...
        >
          ...
          {isMulti &&
            (imgList && imgList.length > 0 ? <Button type="text">重新上传</Button> : uploadButton)}
        </Upload>
        {isMulti && imgList && imgList.length > 0
          ? imgList.map((item) => (
              <img
                src={(process.env.NODE_ENV === 'local' ? properties.baseUrl : '') + item}
                alt="avatar"
                style={{ width: 52, height: 52, marginRight: 3 }}
              />
            ))
          : ''}
      </div>
