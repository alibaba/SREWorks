...
export default class OamWidgets extends React.Component {
  constructor(props) {
    ...
    let children = metaData.children || [],
      routeData = {},
      initNodeParams = Object.assign(
        { __currentUser__: currentUser },
        defaultNodeParams,
        urlSearchParams,
      )
  ...
  loadPageContextParams = () => {
    ...
    pageDataSource = JSON.parse(
      util.renderTemplateString(JSON.stringify(pageDataSource), {
        ...nodeParams,
        ...defaultNodeParams,
        ...urlParams,
      }),
    )
    let { url, params = {}, method = 'get', mode = 'async' } = pageDataSource,
      req = null
  ...
  render() {
    let { widgets, layout, nodeId, metaData, urlRequireNodeId, ...contentProps } = this.props,
      { routeData, preCheckPass, awaitPageParams } = this.state
    ...
    let children = (metaData && metaData.children) || [],
      urlSearchParams = util.getUrlParams(),
      defaultTab,
      nowPath = this.props.history.location.pathname
    if (children.length > 0) {
      ...
      <div className={bgFill ? 'globalBackground' : ''} style={{ padding: bgFill ? 8 : 0 }}>
        ...
        {children.length > 0 ? (
          <div key={'__node_menus_' + nodeId}>
            {menus.length > 0 ? (
              <Menu selectedKeys={[defaultTab]} mode="horizontal" className="t-menu">
                {menus}
              </Menu>
            ) : null}
