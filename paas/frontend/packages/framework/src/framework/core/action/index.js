    ...
    if (widgetModel.type === 'STEP_ACTION' && blocks) {
      ...
      for (let b = 0; b < blocks.length; b++) {
        ...
        if (elements[0] && elements[0].type === 'FluidGrid') {
          ...
          rows.forEach((row) => {
            actionElement = row.elements.filter(
              (elementMeta) => elementMeta.type === 'ACTION' || elementMeta.type === 'STEP_FORM',
            )[0]
          })
        } else {
          actionElement = elements.filter(
            (elementMeta) => elementMeta.type === 'ACTION' || elementMeta.type === 'STEP_FORM',
          )[0]
        }
        if (actionElement) {
  ...
  render() {
    ...
    if (!actionData) {
      return (
        ...
        >
          <h3>{widgetModel.type === 'STEP_ACTION' ? '请设置分步操作' : '请定义步骤表单'}</h3>
        </div>
      )
    }
    return (
