...
public class AddonInstanceTaskRunnerNode extends AbstractLocalNodeBase {
    ...
    public DagInstNodeRunRet run() throws Exception {
        ...
        ComponentSchema addonSchema = addon.applyInstance(request);

        // 创建 addon instance
        AddonInstanceDO addonInstance = AddonInstanceDO.builder()
        ...
        log.info("addon instance has applied|addonInstaceId={}|namespaceId={}|addonId={}|" +
                        "addonName={}|addonAttrs={}", addonInstance.getAddonInstanceId(), request.getNamespaceId(),
                request.getAddonId(), request.getAddonName(),
                JSONObject.toJSONString(request.getAddonAttrs()));
