...
import { Card, Tabs, Collapse, Row, Col, Switch } from 'antd'
...
import { getLegacyWidgetMeta } from '../../framework/WidgetRepository'
...
export default class ElementEditor extends React.Component {
  ...
  saveEditor = () => {
    let { widgetMeta, widgetModel } = this.state,
      values = this.saveHandleHook.validate()
    if (values) {
  ...
  render() {
    ...
    let CommonTabContent = null,
      tabContentStyle = { height: 'calc(45vh - 10px)', overflowY: 'auto', overflowX: 'none' }
    // if(this.isFilter){
    ...
    if (widgetModel.type === 'CustomComp') {
      ...
      if (widgetMeta && widgetModel.name) {
        widgetMeta['configSchema']['schema']['properties']['compName']['initValue'] =
          widgetModel.name
        widgetMeta['configSchema']['schema']['properties']['compDescribtion']['initValue'] =
          (widgetModel.info && widgetModel.info.description) || ''
        let obj = {
      ...
      <div>
        <Tabs
          ...
          renderTabBar={(props, DefaultTabBar) => {
            return (
              <div>
                <h4 style={{ float: 'left', marginRight: 36, marginTop: 9, fontSize: 18 }}>
                  {editTitle}
                </h4>
                <DefaultTabBar {...props} />
              </div>
          ...
          tabBarExtraContent={
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <div>
                ...
                >
                  {isJsonEdit ? '源码编辑' : '可视化编辑'}
                </div>
                <Switch
                  style={{ marginRight: '30px', display: 'inline-block' }}
                  defaultChecked
                  checked={isJsonEdit}
                  onChange={this.switchUpdateRule}
                />
        ...
        >
              ...
              <div style={tabContentStyle}>
                ...
                {this.isFilter && (
                  <FilterSetting
                    ...
                    onValuesChange={(changedField, allValue) => this.setCommonConfig(allValue)}
                  />
                )}
                {this.isAction && (
                  <ActionSetting
          ...
          {this.isFilter && !isJsonEdit && (
            <TabPane key="filter" tab={<span>过滤项</span>}>
              <div style={tabContentStyle}>
                {this.isTabFilter && (
                  <TabFilterSetting
                    onChange={(parameters) => this.setCommonConfig({ parameters: parameters })}
                    parameters={this.commonConfig.parameters || []}
                  />
                )}
                {!this.isTabFilter && (
                  <FormEditor
          ...
          {!this.isAction &&
            ...
            widgetMeta &&
            !isJsonEdit &&
            ((widgetMeta.configSchema.schema &&
              Object.keys(widgetMeta.configSchema.schema).length) ||
              ...
              <TabPane key="custom" tab={<span>组件属性</span>}>
                <Row style={tabContentStyle}>
                  <Col span={16}>
                    <div>
                      {widgetMeta.configSchema.schema &&
                        Object.keys(widgetMeta.configSchema.schema).length > 0 && (
                          <CustomSetting
                            widgetModel={widgetModel}
                            config={Object.assign({}, this.commonConfig)}
                            widgetMeta={widgetMeta}
                            onValuesChange={(changedField, allValue) =>
                              this.setCommonConfig(changedField)
                            }
                          />
                        )}
                      {widgetMeta.configSchema.supportItemToolbar && (
                        <Collapse>
                          <Panel header="数据项工具栏" key="row_action">
                            <ToolbarSetting
                              noFilter={true}
                              noHelp={true}
                              configIndex="itemToolbar"
                              widgetModel={widgetModel}
                              config={Object.assign({}, this.commonConfig)}
                              onValuesChange={(changedField, allValue) =>
                                this.setCommonConfig({ itemToolbar: allValue })
                              }
                            />
                          </Panel>
                        </Collapse>
                        ...
                        <Collapse>
                          <Panel header="大屏图表配置" key="row_action">
                            <ScreenSetting
                              ...
                              config={Object.assign({}, this.commonConfig)}
                              onValuesChange={(changedField, allValue) =>
                                this.setCommonConfig({ chartDisplayConfig: allValue })
                              }
                            />
                          </Panel>
                        </Collapse>
          ...
          {!this.isAction &&
            !this.isFilter &&
            !(hasWrapper && hasWrapper === Constants.CARD_WRAPPER_NONE) &&
            widgetMeta &&
            widgetMeta.configSchema.supportToolbar &&
            !isJsonEdit && (
              <TabPane key="toolbar" tab={<span>工具栏</span>}>
                <div style={tabContentStyle}>
                  <ToolbarSetting
                    ...
                    config={Object.assign({}, this.commonConfig)}
                    onValuesChange={(changedField, allValue) =>
                      this.setCommonConfig({ toolbar: allValue })
                    }
                  />
                </div>
              </TabPane>
            ...
            <TabPane tab="组件数据源" key="datasource">
              <div style={tabContentStyle}>
                <DataSourceEditor
                  ...
                  dataFormatter={
                    widgetMeta && widgetMeta.configSchema ? widgetMeta.configSchema.dataMock : false
                  }
