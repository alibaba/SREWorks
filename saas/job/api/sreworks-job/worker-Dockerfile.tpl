...
RUN sed -i 's/dl-cdn.alpinelinux.org/{{ APK_REPO_DOMAIN }}/g' /etc/apk/repositories && apk add build-base curl && apk add bash python3 && apk add python3-dev
RUN pip3 config set global.index-url {{ PYTHON_PIP }} && pip config set global.trusted-host {{ PYTHON_PIP_DOMAIN }}
