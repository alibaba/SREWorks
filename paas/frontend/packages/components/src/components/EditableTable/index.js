...
class EditableTable extends React.Component {
  ...
  handleSave = (record) => {
    let { form, editStatusChanged } = this.props,
      { rowData } = this.state
  ...
  handleAdd = () => {
    let newRow = { __edit_key__: uuidv4() },
      { onChange, editStatusChanged } = this.props
    //增加新增行存在初始值的显示
  ...
  render() {
    let { form, enableRemove, enableAdd, enableScroll, isCanRemove, ...tableProps } = this.props,
      { editingRow, rowData } = this.state
          ...
          render: (text, record, index) => {
            ...
            if (editingRow && record.__edit_key__ === editingRow.__edit_key__) {
              return FormElementFactory.createFormItem(
                Object.assign({}, column.editProps, {
                  name: `${record.__edit_key__}.${column.editProps.name}`,
                  initValue: values,
                }),
                form,
                formItemLayout,
            ...
            if (column.editProps.type === FormElementType.SELECT) {
              if (
                record[column.editProps.name + 'Option'] &&
      ...
      render: (text, record) => {
        if (editingRow.__edit_key__ === record.__edit_key__) {
          return (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              <a onClick={() => this.handleSave(record)} style={{ marginRight: 8 }}>
                {localeHelper.get('manage.taskplatform.common.save', '保存')}
              </a>
              <a onClick={() => this.handleCancel(record)}>
                {localeHelper.get('ButtonCancel', '取消')}
              </a>
            </div>
          )
        }
        return (
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
            ...
            {enableRemove && !record[isCanRemove] && (
              <a onClick={() => this.handleRemove(record)} style={{ color: 'red' }}>
                <DeleteOutlined />
              </a>
            )}
          </div>
                    ...
                    >
                      ...
                      <span>
                        <a onClick={() => this.handleAdd()}>
                          ...
                          {localeHelper.get('manage.taskplatform.common.new', '添加')}
                        </a>
                      </span>
                    </div>
                  )
