 ...
 */
class TraitNodePort implements TraitHandler {

    private static final Logger log = LoggerFactory.getLogger(TraitNodePort.class)
     ...
     * 当前内置 Handler 名称
     */
    public static final String NAME = "nodeport.trait.sreworks.io/v1beta1"
    ...
    TraitExecuteRes execute(TraitExecuteReq request) {
         ...
         */
        log.info("start execute nodeport trait {}", request.getSpec().toJSONString())
        ...
        String nodePortServiceName = request.getSpec().getString("name") ? request.getSpec().getString("name"): name + "-" + "nodeport"
        String clusterId = request.getComponent().getClusterId()
        Integer nodePort = request.getSpec().getInteger("nodePort")
        ...
        /**
         * 2. generate nodeport service and apply to k8s cluster
         */
        log.info("start to apply nodeport service by trait {}", request.getSpec().toJSONString())
        ...
        servicePortBuilder.withNodePort(nodePort);
        ...
        Service current = client.services().inNamespace(namespace).withName(nodePortServiceName).get();
        if (current == null) {
            Service result = client.services().createOrReplace(service);
            log.info("cr yaml has created in kubernetes|cluster={}|namespace={}|nodePortServiceName={}" +
                    "result={}", clusterId, namespace, nodePortServiceName, JSONObject.toJSONString(result));
        }else{
            Service result = client.services().inNamespace(namespace).withName(nodePortServiceName).patch(service);
            log.info("cr yaml has patched in kubernetes|cluster={}|namespace={}|nodePortServiceName={}" +
                    "result={}", clusterId, namespace, nodePortServiceName, JSONObject.toJSONString(result));
        }
