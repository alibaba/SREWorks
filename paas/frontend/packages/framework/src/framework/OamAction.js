...
import { Spin, Button, Card, Modal, Tooltip, Drawer, message, Alert, Input, Radio } from 'antd'
import SimpleForm from '../../components/FormBuilder/SimpleForm'
import Action from './model/Action'
import APIAction from './model/APIAction'
import actionService from '../services/actionService'
import { connect } from 'dva'
import { safeEval, localeHelper, util, httpClient } from '@sreworks/shared-utils'
...
import { JSXRender, FormElementType } from '@sreworks/components'
...
class OamAction extends React.Component {
  constructor(props) {
    super(props)
    let { actionData, userInfo, product, nodeParams, userParams, actionParams } = props,
      action = null
    let actionInitParams = Object.assign(
      {},
      nodeParams,
      util.getUrlParams(),
      userParams,
    ...
    let configMetaConf = actionData.config,
      runtimeData = actionData.runtimeData,
      configMeta
    if (configMetaConf) {
    ...
    if (scenes && scenes.length) {
      ...
      scenes.forEach((s) => {
        ...
        Object.keys(svs).forEach((sk) => {
          if (
            (urlParams[sk] && svs[sk] !== urlParams[sk]) ||
            (!urlParams.hasOwnProperty(sk) && svs[sk])
          ) {
            isDefault = false
          }
        })
  ...
  previewAction = () => {
    let { nodeParams, userParams, actionParams } = this.props,
      { configMeta } = this.state
    //配置有远程获取参数接口
    if (configMeta && configMeta.sourceApi) {
      ...
      if (sourceApi.length > 3) {
        ...
        if (beforeSourceHandler && beforeSourceHandler.length > 80) {
          preParamsSet = safeEval('(' + beforeSourceHandler + ')(nodeParams)', {
            nodeParams: preParamsSet,
          })
        }
        actionService
          .actionPreview(sourceApi, preParamsSet, configMeta.sourceMethod)
          .then((preview) => {
            if (afterSourceHandler && afterSourceHandler.length > 80) {
              let tranPreview = safeEval('(' + afterSourceHandler + ')(resData)', {
                resData: preview,
              })
  ...
  handleBeforeOpen = (remoteParams = {}) => {
    let { nodeParams, userParams, actionParams = {} } = this.props,
      { configMeta } = this.state
    let { beforeHandler } = configMeta,
      preExecute = this.preExecute
    //定义了前置处理器,因内置了自定义的模板函数,因此只有大于模板长度再进行计算
    if (beforeHandler && beforeHandler.length > 150) {
      let result = safeEval('(' + beforeHandler + ')(mergeNodeParams)', {
        mergeNodeParams: Object.assign({}, nodeParams, userParams, actionParams, remoteParams),
      })
      let { message, transformParams, pass, dynamicItems, feedbacks } = result,
        beforeResults = {}
      //存在参数转换或者动态表单项
      if (transformParams) {
    ...
    if (action.refresh === 'Y' || refresh) {
      this.setState(
        ...
        () => {
          ...
          dispatch({
            type: 'node/updateParams',
            paramData: { ___refresh_timestamp: new Date().getTime() },
          })
          this.onClose()
        },
      )
  ...
  executeAction = (params, actionButton) => {
    //如果是输出类型的直接调用输出
    let { action, remoteParams, dispalyItems } = this.state,
      {
        nodeParams,
        userParams,
        actionParams,
    ...
    if (action.actionType === Action.OUTPUT) {
      ...
      this.setState(
        ...
        () => {
          ...
          let outputData = { ...params },
            parameters = action.parameterDefiner.getParameters()
    ...
    let allParams = Object.assign({}, nodeParams, userParams, actionParams, remoteParams, params),
      ...
      if (name !== '__approveCountersign' && label) {
        ...
        detailDescription[label] =
          value instanceof moment
            ? value.format(window.APPLICATION_DATE_FORMAT || 'YYYY-MM-DD HH:mm:ss')
            : value
      }
    })
    //action详情参数的kv
    ...
    action
      .execute(allParams, params, actionButton)
      .then((result) => {
        let feedbacks = result && result.feedbacks,
          receiveParams = (result && result.params) || {},
          { modalWidth } = actionButton || {},
          feedbackTitle = result && result.feedbackTitle
        if (feedbacks && feedbacks.length > 0) {
            ...
            if (modalInfo.pass) {
              ...
              if (actionButton && actionButton.confirmUrl) {
                return executeAction(
                  Object.assign({}, params, receiveParams),
                  Object.assign({}, actionButton, { url: actionButton.confirmUrl }),
                )
              } else {
                closeAndRefresh()
                return Promise.resolve()
          ...
          if (action.responseHandler && action.responseHandler.length > 53) {
            responseText = safeEval('(' + action.responseHandler + ')(response)', {
              response: result,
            })
          }
  ...
  preExecute = () => {
    ...
    let { action, remoteParams, dynamicItems } = this.state,
      { nodeParams, userParams, actionParams, displayType } = this.props,
    ...
    action.getInputParamDef(approveGroupName).then((defs) => {
      let dispalyItems = defs,
        initParams = {},
        effectItemMapping = false
      ...
      dispalyItems.sort(function (a, b) {
  ...
  handleCustomButton = (params, button) => {
    if (!params) return
    let verifyFunction = button.verifyFunction,
      executeAction = this.executeAction,
      closeAndRefresh = this.closeAndRefresh
    //粗滤,初始化函数就有120的字符,因此认为小于120个字符的校验函数为无效函数
    if (verifyFunction && verifyFunction.length > 120) {
      let { remoteParams } = this.state,
        { nodeParams, userParams, actionParams } = this.props,
        urlParams = util.getUrlParams(),
        { modalWidth } = button || {}
      let result = safeEval('(' + verifyFunction + ')(nodeParams,formValues)', {
        nodeParams: Object.assign(
          {},
          nodeParams,
          urlParams,
          userParams,
  ...
  handleAddScene = (values) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        ...
        let hasExist = false,
          newScene = { title: sceneName, values: values }
        ...
        let params = {
          ...
          nodeTypePath: nodeParams.__nodeTypePath__,
          runtimeData: runtimeData,
        }
        return httpClient.put(
          'gateway/v2/common/productops/frontend/nodeTypePaths/element/config',
          params,
        )
      },
    })
  ...
  handleDeleteScene = (sceneTitle) => {
    let cusScenes = this.runtimeData.scenes,
      { action } = this.state,
      { nodeParams } = this.props
  ...
  handleFilterReset = () => {
    let values = this.action_form_container.getFieldsValue(),
      { dispalyItems } = this.state,
      newUrlParams = util.getUrlParams(),
      { history } = this.props
  ...
  handlePreSubmitAction = (params) => {
    ...
    let { action, remoteParams } = this.state,
      { nodeParams, userParams, actionParams } = this.props
    //合并参数来源一同传递给作业,输入参数及自定义数据源优先级最高
    let allParams = Object.assign({}, nodeParams, userParams, actionParams, remoteParams, params),
      executeAction = this.executeAction,
    ...
    if (beforeSubmitHandler && beforeSubmitHandler.length > 150) {
      let result = safeEval('(' + beforeSubmitHandler + ')(nodeParams,formValues,httpClient)', {
        nodeParams: allParams,
        formValues: params,
  ...
  handleItemEffect = (changedValues, allValues) => {
    ...
    if (this.action_form_container) {
      let { remoteParams } = this.state,
        { nodeParams, userParams, actionParams } = this.props
      let { effectItemMapping } = this.state,
        allNodeParams = Object.assign({}, nodeParams, userParams, actionParams, remoteParams)
      let changedFields = Object.keys(changedValues)
      changedFields.forEach((field) => {
        //表单项存在effect函数进行函数执行
        if (effectItemMapping[field]) {
          let result = safeEval(
            '(' + effectItemMapping[field] + ')(nodeParams,formValues,httpClient)',
            { nodeParams: allNodeParams, formValues: allValues, httpClient: httpClient },
          )
  ...
  render() {
    ...
    let labelSpan = sizeDef.labelSpan,
      { formLayout = 'horizontal' } = action
    if (action.labelSpan) {
    ...
    let baseStyle = { display: 'flex', justifyContent: 'center' },
      fixedStyle =
        this.props.pageLayoutType === 'CUSTOM'
          ? {
              display: 'flex',
              justifyContent: 'center',
              position: 'fixed',
              borderTop: '1px solid #e8e8e7',
              top: computedHeight - 150,
              right: 0,
              width: computedWidth,
              height: 50,
              paddingTop: 14,
    ...
    if (showHistory) {
      ...
      if (
        displayType !== 'form' &&
        displayType !== 'filter' &&
        displayType !== 'filterBar' &&
        mode !== 'step' &&
        displayType !== 'mixFilterBar' &&
        displayType !== 'tabFilter'
      ) {
        actionButtons.push(
          <Button size="small" key="_a_b_cancel" onClick={this.onClose}>
            {localeHelper.get('manage.taskplatform.common.cancel', '取消')}
          </Button>,
        )
      }
      let onFormSubmit = this.executeAction,
        formVisible = true,
        { remoteParams } = this.state,
        { nodeParams, userParams, actionParams } = this.props,
        urlParams = util.getUrlParams()
      let { beforeSubmitHandler } = action
      if (beforeSubmitHandler && beforeSubmitHandler.length > 150) {
      ...
      if (
        ...
        buttons.forEach((button, index) => {
          let expression = button.expression,
            visible = true
          if (expression) {
          ...
          if (visible) {
            actionButtons.push(
              <Button
                ...
                type="primary"
                onClick={() => {
                  this.handleCustomButton(this.preCustomButtonSubmit.validate(), button)
                }}
              >
                {button.name}
              </Button>,
            ...
            actionButtons.push(
              <Tooltip key="_a_b_search" title={localeHelper.get('Search', '搜索')}>
                <Button
                  ...
                  type="primary"
                  icon={<SearchOutlined />}
                  onClick={() => this.executeAction.validate()}
                />
              </Tooltip>,
            )
            ...
            if (action.canExecute()) {
              actionButtons.push(
                <Button
                  ...
                  onClick={() => this.executeAction.validate()}
                >
                  {action.actionType === Action.OUTPUT
                    ? localeHelper.get('ButtonOK', '确定')
                    : action.submitLabel
                    ? action.submitLabel
                    : localeHelper.get('manage.taskplatform.common.execute', '执行')}
                </Button>,
        ...
        if (
          ...
          actionButtons.push(
            <Button
              ...
              onClick={this.handleFilterReset}
            >
              {localeHelper.get('manage.flow.reset', '重置')}
            </Button>,
          )
          if (addScene)
            actionButtons.push(
      ...
      if (displayType === 'filter') {
        ...
        return (
          <FilterBar
            ...
            extCol={action.actionType === Action.OUTPUT ? actionButtons : null}
            ...
            <div>
              <Spin spinning={loading}>
                ...
                {action.actionType === Action.OUTPUT ? null : (
                  <div
                    className="globalBackground"
                    style={action.isDrawer ? fixedStyle : baseStyle}
                  >
                ...
                {scenes.length === 0 || showScenes === false ? null : (
                  <div style={{ marginTop: 12 }}>
                    <Radio.Group
                      ...
                      buttonStyle="solid"
                      onChange={(e) => this.handleSceneChanged(e.target.value)}
                    >
                      {scenes.map((scene, index) => {
                        return (
                          ...
                          >
                            ...
                            (action.scenes || []).filter((ac) => ac.title === scene.title)
                              .length === 0 ? (
                              <a onClick={() => this.handleDeleteScene(scene.title)}>
    ...
    if (action.displayType === 'modal') {
      return (
        <div style={{ marginRight: 16 }}>
          {mode !== 'custom' ? (
            <a disabled={!execInfo.executeAble} onClick={this.previewAction}>
              <LegacyIcon type={action.icon || 'tool'} style={{ marginRight: 8 }} />
              {action.label}
            </a>
          ) : null}
          <Modal
