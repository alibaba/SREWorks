...
class ActionForm extends Component {
  ...
  render() {
    const { action, buildId } = this.state,
      { mode } = this.props
    if (!action) {
      return <Spin />
    }
    return (
      <OamAction
        {...this.props}
        showSearch={mode.config.showSearch}
        advanced={mode.config.advanced}
        showScenes={mode.config.showScenes}
        addScene={mode.config.addScene}
        key={buildId || action.id || action.elementId}
        actionId={action.id || action.elementId}
        actionData={action}
        mode="custom"
        displayType={this.props.displayType || 'form'}
      />
    )
  }
}
