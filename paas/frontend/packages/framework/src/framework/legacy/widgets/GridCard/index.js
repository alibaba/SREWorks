...
class GridCard extends React.Component {
  constructor(props) {
    ...
    if (
      ...
      this.props.widgetConfig.dataSourceMeta.api
    ) {
    ...
    runtimeUrl &&
      ...
      this.loadAllData(runtimeUrl, filterParams).then((respData) => {
        if (afterResponseHandler && afterResponseHandler.length > 50) {
          respData = safeEval('(' + afterResponseHandler + ')(respData,nodeParams,httpClient)', {
            respData: respData,
        ...
        let dataIndex = mode.config.dataIndex,
          tdata = (respData && respData.items) || respData || []
  ...
  loadAllData = (api, filterParams = {}) => {
    ...
    if (!api) {
      ...
      let confParams = Object.assign(_.get(api, 'params') || {}, util.getUrlParams(), {
      ...
      if (beforeRequestHandler && beforeRequestHandler.length > 50) {
        let funcParams = safeEval('(' + beforeRequestHandler + ')(nodeParams)', {
          nodeParams: nodeParams,
        })
  ...
  componentDidUpdate(prevProps) {
    const { mode, parameters, nodeParams } = this.props,
      { apiConf } = this.state
    if (!_.isEqual(prevProps.nodeParams, this.props.nodeParams)) {
      //前端api方式配置
    ...
    return (
      <div className="description-wrap">
        {columns.map((column, index) => {
          return (
            <div key={index} className="description-item" style={style}>
  ...
  descriptionRenderBackup = (item) => {
    ...
    let { columns } = this.state
    return (
      <div className="description-wrap-backup">
        <div style={{ width: '50%' }}>
          {columns &&
            columns.slice(1).map((column, index) => {
              return (
                <div key={index} className="description-item-backup" style={style}>
          ...
          {columns &&
            ...
            [columns[0]].map((column, index) => {
              return (
                <div
                  key={index}
                  className="description-item-backup"
                  style={{ justifyContent: 'center', padding: 'auto 0' }}
                >
                  <div className="description-item-title" style={{ textAlign: 'center' }}>
                    {column.label}
                  </div>
                  <div
                    className="description-item-value-out"
      ...
      <Spin spinning={loading}>
        <List
          ...
          renderItem={(item, r) => {
            ...
            let itemRowAction =
              item.rowActions && !!item.rowActions.actions.length ? item.rowActions : rowActions
            return (
              <List.Item key={r}>
                <Card hoverable actions={this.actionsRender(itemRowAction, item)}>
                  <Meta
                    avatar={avatar}
                    title={title || item.label || item.name}
                    description={
                      layoutSelect === 'advance'
                        ? this.descriptionRenderBackup(item)
                        : this.descriptionRender(item)
                    }
                  />
