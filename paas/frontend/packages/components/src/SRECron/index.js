        ...
        <Form.Item key={model.name} style={{ display: 'inline-block' }}>
          {
            <div>
              <Input
                style={{ width: 170, marginRight: 2 }}
                initValue={model.initValue}
                value={cronText}
                placeholder={localeHelper.get('cron.expression', '定时表达式')}
              />
              <Button onClick={() => this.setState({ cronModalVisible: true })}>
                {localeHelper.get('cron.autogen', '帮我配置')}
              </Button>
              <Button
                onClick={() =>
                  httpClient
                    .get(`gateway/sreworks-job/quartz/checkCron?cron=${cronText}`)
                    .then((res) => {
                      if (res === 'OK') {
                        httpClient
                          .get(
                            `gateway/sreworks-job/quartz/getNextTriggerTime?cron=${cronText}&size=5`,
                          )
                          .then((resSub) => {
                            Modal.success({
                              title: '校验正确',
                                    ...
                                    <Col span={13}>
                                      {resSub.map((lit) => (
                                        <div style={{ marginBottom: 8 }}>
                                          {moment(lit).format('YYYY-MM-DD HH:mm:ss')}
                                        </div>
                                      ))}
                                    </Col>
                                  </Row>
              ...
              >
                {localeHelper.get('cron.checkExpression', '校验表达式')}
              </Button>
            </div>
          }
        </Form.Item>
        <Modal
          title={localeHelper.get('runOnTime', '定时运行')}
          ...
          footer={
            <div>
              ...
              <Button
                onClick={() =>
                  this.setState({ cronModalVisible: false }, () =>
                    setFieldsValue({ [targetName]: this.state.cronText }),
                  )
                }
              >
