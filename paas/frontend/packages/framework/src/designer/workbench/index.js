...
export default class Workbench extends React.Component {
  ...
  saveAsBlocks = (templateServiceType) => {
    let paramsArray = []
    this.cloneGroupData[0] &&
      this.cloneGroupData[0].items.forEach((item) => {
        let pageModelItem = new PageModel(item)
        let pageJsonItem = pageModelItem.toJSON()
        let blockConfig = {
          label: pageModelItem.label,
          name: pageModelItem.name || pageModelItem.id,
          category: pageModelItem.category,
          tags: pageModelItem.tags,
        }
        let obj = {
          ...item,
          ...pageJsonItem,
  ...
  render() {
    let {
      ...
      { stageHeight = 660, style, ...otherProps } = this.props
    ...
    return (
      <Layout className="abm_frontend_designer" style={{ height: stageHeight }}>
          ...
          <div style={{ display: menuFold ? 'none' : '' }}>
            ...
            <div style={{ overflowY: 'auto', height: contentHeight }}>
              <MenuNavigation
                ...
                ref={this.childRef}
                appId={this.props.appId || 'front-dev'}
                menuFold={menuFold}
        ...
        <div
          onClick={(e) => {
            this.setState({ menuFold: !menuFold }, () => this.handleContentResize())
          }}
        ...
        <Content className={!nodeTypeId ? 'globalBackground' : ''} style={{ height: stageHeight }}>
          <Spin spinning={contentLoading}>
              ...
              <Layout>
                ...
                >
                  ...
                  <div style={{ display: tabFold ? 'none' : '' }}>
                    ...
                    <div
                      className="node-navigation"
                      style={{ overflowY: 'auto', height: contentHeight, overflowX: 'hidden' }}
                    >
                      <NodeNavigation
                        mainPageClick={this.onMainPageClick}
            ...
            {!nodeTypeId && (
              <div className="no-data globalBackground">
                <div className="img-wrapper">
                  <img width={200} src={noDataImg} />
                </div>
                <div className="no-data-text">暂无数据，请点击左边菜单节点树进行配置哦</div>
              </div>
            )}
          </Spin>
        </Content>
