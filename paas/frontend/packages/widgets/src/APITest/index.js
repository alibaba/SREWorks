...
import { JSONEditor } from '@sreworks/components'
...
const APITest = (props) => {
  let { widgetConfig = {} } = props,
    sceneCodeProp = 'default',
    detectorCodeProp = 'drilldown'
  ...
  const getDefaultInput = () => {
    httpClient.get(`gateway/aiops/aisp/${sceneCode}/${detectorCode}/input`).then((res) => {
      setDefaultInput(res)
    })
  }
  ...
  const copyButton = (props, text) => {
    return (
      <Button type={props.type} size={props.size} style={props.style} onClick={() => copy(text)}>
        复制
      </Button>
    )
  }
  ...
  const handleSubmit = (codeType) => {
    httpClient
      .post(`/gateway/aiops/aisp/${sceneCode}/${detectorCode}/code`, {
        codeType,
        input: inputParams,
        url: testLink,
      })
    ...
    <div style={{ marginRight: 20 }}>
      ...
      <CardWrapper
        ...
        extButton={
          <div>
            {copyButton(
              { style: { marginRight: 10 }, size: 'small', type: 'primary' },
              inputParams,
            )}
            <Button size="small" type="primary" onClick={paste}>
              粘贴
            </Button>
          </div>
        }
      >
        <Form onValuesChange={onChange} initialValues={{ inputParams }}>
          <Form.Item name="inputParams">
            {!refresh && (
              <JSONEditor
                model={{ initValue: inputParams, height: 180, defModel: { disableShowDiff: true } }}
              />
            )}
          </Form.Item>
        </Form>
      ...
      <CardWrapper
        title="输出参数"
        extButton={<div>{copyButton({ size: 'small', type: 'primary' }, outputParams)}</div>}
      >
        {!refreshOutput && (
          <JSONEditor
            onChange={handleChangeOutput}
            model={{ initValue: outputParams, height: 180, defModel: { disableShowDiff: true } }}
          />
        )}
      </CardWrapper>
      ...
      {outputParams && (
        <div>
          ...
          <span
            style={{
              color: (serviceMap[serviceCode] && serviceMap[serviceCode].color) || '#ff4d4f',
            }}
          >
            {serviceCode}
          </span>
          &nbsp;&nbsp;耗时：<span>{runTime}ms</span>
        </div>
      )}
      ...
      <div style={{ textAlign: 'right' }}>
        <Button style={{ marginRight: 10 }} type="primary" onClick={() => handleSubmit('curl')}>
          生成CURL命令
        </Button>
        <Button style={{ marginRight: 10 }} type="primary" onClick={() => handleSubmit('python')}>
          生成Python代码
        </Button>
        <Button type="primary" onClick={() => handleSubmit('java')}>
          生成Java代码
        </Button>
      </div>
