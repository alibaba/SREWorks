...
import { OamContent } from '@sreworks/framework'
import { util } from '../../utils/utils'
import { NodeContent } from '@sreworks/framework'
...
class ContentWithMenus extends React.Component {
    ...
    contentRoutes = (
      <div>
        {routeData.children.map(
          ({ path, children, component, type, sync, nodeId, ...dynamics }, key) => {
              ...
              return (
                <Route
                  ...
                  component={() => (
                    <NodeContent
                      {...this.props}
                      routeData={childrenNode}
                      nodeData={childrenNode}
                      nodeId={nodeId}
                      nodeConfig={childrenNode.config}
                    />
                  )}
                />
              )
                ...
                return (
                  <Route
                    ...
                    component={() => (
                      <OamContent
                        {...this.props}
                        menuRouteData={routeData.children[key]}
                        nodeId={nodeId}
                        urlRequireNodeId={false}
                        nodeConfig={routeData.children[key].config}
                      />
                    )}
                  />
                )
        ...
        {routeData.children && routeData.children[0] ? (
          <Route
            exact
            path={routeData.path}
            render={() => <Redirect to={routeData.children[0].path} />}
          />
        ) : null}
      </div>
    )
    meunItems = routeData.children
      .map(({ path, children, name, icon, component, label, config = {} }) => {
            ...
            <Menu.Item key={path}>
              {redirect && redirect.path ? (
                ...
                <Link
                  to={{
                    pathname: path,
                    search: util.objectToUrlParams(
                      Object.assign(urlSearchParams || {}, fromAppKeyParams),
                    ),
  ...
  render() {
    let { routeData } = this.props,
      lp = this.props.history.location.pathname,
      selectKeys = []
    ...
    if (menuBarHeader && menuBarHeader.indexOf && menuBarHeader.indexOf('$') > -1) {
      disPlayMenuBarHeader = (
        <JSXRender
          jsx={util.renderTemplateString(menuBarHeader, Object.assign({}, util.getUrlParams()))}
        />
      )
    }
    return (
      <div>
        {meunItems.length ? (
          <div
            className="content-top-menu-bar globalBackground"
