...
function DescribtionCard(props) {
  ...
  let {
    cardTitle = '工作负载',
    cardContent = '理解 Pods，Kubernetes 中可部署的最小计算对象，以及辅助它运行它们的高层抽象对象',
    imgUrl,
    describeLayout = 'cardLayout',
    cardActions = '<a href="#">详情</a>',
  } = widgetConfig
  const randomImgArr = [
  ...
  if (describeLayout === 'cardLayout') {
    return (
      ...
      >
        <h2 className={style.describeTitle}>{cardTitle}</h2>
        <p className={style.describeContent}>{cardContent}</p>
      </Card>
    )
  }
  return (
    <div className={style.describeCard}>
      <img
        className={style.describeImg}
        alt="icon-0"
        width="88"
        src={imgUrl || randomImgArr[Math.floor(Math.random() * 10)]}
      ></img>
      <h2 className={style.describeTitle}>{cardTitle}</h2>
      <p className={style.describeContent}>{cardContent}</p>
      <JSXRender className={style.describeLink} jsx={cardActions || '<a href="#">详情</a>'} />
    </div>
  )
}
