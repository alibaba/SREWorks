...
function getTargetUrl(config) {
  ...
  if (config.url.startsWith('http:') || config.url.startsWith('https:')) {
    // 如果配置的url以http或https开头，则不在前面加apiEndpoint
...
function showErrorDetails(res) {
  ...
  let requestDetails = syntaxHighlight(res.config),
    respDetails = syntaxHighlight(res.data)
  ...
  if (res.data && res.data.message_type === 'success') {
    return (
      <div style={{ fontSize: 12, marginLeft: -38 }}>
        <span style={{ wordWrap: 'break-word', wordBreak: 'break-all' }}>{res.data.message}</span>
        <br />
      </div>
    )
  }
...
request.interceptors.response.use(
  (res) => {
    //请求成功后,等于200默认业务成功直接返回数据,其他的code分发到模块自己处理
    if (res.status === 200) {
      let resData = res.data
      if (resData.code) {
        //有data code的场景启动公共拦截及提示
        ...
        if (resData.code === 200) {
          ...
          notification.warning({
            message: 'WARNING',
            description: (
              <div style={{ fontSize: 12, marginLeft: -42 }}>
                {'No Permission,Please contact administrator,message:' + resData.message}
              </div>
            ),
          })
    ...
    if (err && err.response) {
      ...
      if (errorStatus === 401) {
        if (properties.envFlag === properties.ENV.Internal) {
          ...
          showConstExceptionNotification(
            'Error',
            'Not logged in or no permission,Please contact administrator',
          )
          return Promise.reject(err)
        }
        ...
        showConstExceptionNotification(
          'Error',
          'The Service is not found,Please contact administrator',
        )
        return Promise.reject(err)
      ...
      showConstExceptionNotification(
        'Error',
        (err.message ? err.message : 'Unknown error') + ',Please contact administrator',
      )
      return Promise.reject(err)
    }
  },
