...
class HomeLayout extends React.Component {
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    if (!_.isEqual(this.props.params, nextProps.params)) {
      ...
      AppService.getAllProfile(params).then((res) => {
  ...
  render() {
    ...
    if (workspaces[desktopIndex] && workspaces[desktopIndex].background) {
      if (workspaces[desktopIndex].background.indexOf('//') > -1) {
        ...
        backgroundImgeUrl = localImglist.includes(workspaces[desktopIndex].background)
          ? requiredImglist[localImglist.indexOf(workspaces[desktopIndex].background)]
          : properties.baseUrl + workspaces[desktopIndex].background
      }
    } else {
      let localWorkspaces = localStorage.getItem('workspaces')
        ? JSON.parse(localStorage.getItem('workspaces'))
        : []
      if (localWorkspaces.length) {
        backgroundImgeUrl = localImglist.includes(localWorkspaces[desktopIndex].background)
          ? requiredImglist[localImglist.indexOf(localWorkspaces[desktopIndex].background)]
          : properties.baseUrl + localWorkspaces[desktopIndex].background
      }
    }
    // else {
    ...
    return (
      <div
        className="abm-home-page"
        style={{
          backgroundImage: `url(${backgroundImgeUrl})`,
          ...
          <Layout style={{ background: 'transparent' }}>
            <Content className="workspace">
              <Spin spinning={loading}>
                {!loading && <HomeWorkspace widgetConfig={widgetConfig} />}
              </Spin>
              <WorkspaceSetting />
            </Content>
          </Layout>
