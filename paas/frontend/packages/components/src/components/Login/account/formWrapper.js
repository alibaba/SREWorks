...
class CustomFormComponent extends React.Component {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        ...
        if (this.props.aliyunId && this.props.type === 'accountPhone') {
          ...
          accountInfoChange(this.props.aliyunId, values.phone)
            .then((res) => {
              if (res.status === 200) {
          ...
          getAccountCreate(values.aliyunId, values.password, phone, false)
            .then((res) => {
              if (res.status == 200) {
          ...
          this.setState({ inputloading: true })
          upDataPassword(this.props.aliyunId, values.password)
            .then((res) => {
              if (res.status === 200) {
                this.props.onSubmit(res)
              } else if (res.status === 401) {
          ...
          this.setState({ inputloading: true })
          SmsRegister(values.endpoint, values.token)
            .then((res) => {
              if (res.status === 200) {
                this.props.onSubmit(values)
              } else if (res.status === 401) {
  ...
  render() {
    ...
    switch (this.props.type) {
        ...
        formItem = (
          <div>
            <FormItem
              ...
              validateStatus={
                this.upDataPasswordInfoError && this.upDataPasswordInfoError.password
                  ? 'error'
                  : 'success'
              }
              help={
                this.upDataPasswordInfoError && this.upDataPasswordInfoError.password
                  ? this.upDataPasswordInfoError.password
                  : null
              }
            >
              {getFieldDecorator(
                ...
                <Input
                  type="password"
                  placeholder={localeHelper.get(
                    'AccountManagerClouderManagerFormPasswordPlaceholder',
                    '大小写字母、数字、特殊字符，至少三种且长度10到20位',
                  )}
                  onChange={this.updataPasswordChange.bind(this)}
                />,
              )}
            </FormItem>
            <FormItem
              {...formItemLayout}
              label={localeHelper.get(
                'AccountManagerClouderManagerFormConfirmPassword',
                '确认密码',
              )}
              hasFeedback
            >
              {getFieldDecorator('confirm', {
                rules: [
                  {
                    required: true,
                    message: localeHelper.get(
                      'AccountManagerClouderManagerFormConfirmPasswordTip',
                      '请确认密码',
                    ),
                  },
                  {
        ...
        formItem = (
          <div>
            <FormItem
              {...formItemLayout}
              label={localeHelper.get(
                'AccountManagerClouderManagerFormCreateAccountLabel',
                '云账号',
              )}
              validateStatus={
                this.creatInfoError && this.creatInfoError.aliyunId ? 'error' : 'success'
              }
              help={
                this.creatInfoError && this.creatInfoError.aliyunId
                  ? this.creatInfoError.aliyunId
                  : null
              }
            >
              {getFieldDecorator(
            ...
            <FormItem
              ...
              validateStatus={
                this.creatInfoError && this.creatInfoError.password ? 'error' : 'success'
              }
              help={
                this.creatInfoError && this.creatInfoError.password
                  ? this.creatInfoError.password
                  : null
              }
            >
              {getFieldDecorator(
            ...
            <FormItem
              ...
              validateStatus={
                this.creatInfoError && this.creatInfoError.phone ? 'error' : 'success'
              }
              help={
                this.creatInfoError && this.creatInfoError.phone ? this.creatInfoError.phone : null
              }
            >
              {/* 输入手机号 */}
              <InputGroup>
      ...
      case 'register':
        btnName =
          this.props.endpoint && this.props.token
            ? localeHelper.get('Modify', '修改')
            : localeHelper.get('Register', '注册')
        formItem = (
          <div>
            <FormItem
              {...formItemLayout}
        ...
        formItem = (
          <div>
            <FormItem
              ...
              validateStatus={
                this.accountInfoError && this.accountInfoError.phone ? 'error' : 'success'
              }
      ...
      <div>
        <Form onSubmit={this.onSubmit.bind(this)}>
          ...
          <FormItem
            wrapperCol={{
              span: 10,
              offset: (formItemLayout.labelCol.offset || 0) + formItemLayout.labelCol.span,
            }}
          >
            <Button type="primary" loading={this.state.inputloading} htmlType="submit">
              {btnName}
            </Button>
          </FormItem>
        </Form>
  ...
  checkPassword = (rule, value, callback) => {
    ...
    if (value && value !== form.getFieldValue('password')) {
      callback(
        localeHelper.get('AccountManagerClouderManagerFormCheckPassword', '两次输入的密码不一致'),
      )
    } else {
      callback()
    }
