...
import { Divider, Badge, Tooltip, Popover } from 'antd'
...
export default class NotifyCenter extends React.Component {
  ...
  render() {
    const { empId, app } = this.props,
      { todoCount, data } = this.state
    let columns = [
      {
        ...
        render: (text, record) => {
          if (record.source === 'workOrder') {
            return (
              <div>
                <a
                  href={`#/${app}/portal/ticket/detail/changeTicket?requireId=${record.instanceId}`}
                >
                  {localeHelper.get('operation', '处理')}
                </a>
              </div>
            )
          }
          return (
            <div>
              <a href={`#/${app}/portal/ticket/detail/do?id=${record.id}`}>
                {localeHelper.get('operation', '处理')}
              </a>
            </div>
          )
      ...
      <div>
        <Tooltip title={localeHelper.get('common.todo.workorder.list', '待办工单')}>
          <Badge
            ...
            style={{ fontSize: 12 }}
            overflowCount={99}
            className="small-ant-badge-count"
          >
            <Popover
              trigger="click"
              content={
                <div className="oam">
                  ...
                  <div style={{ marginLeft: 120 }}>
                    <Badge count={todoCount} overflowCount={99} className="small-ant-badge-count">
                      <a href={`#/${app}/portal/ticket/processing/list`} style={{ fontSize: 12 }}>
                        查看所有待办
                      </a>
                    </Badge>
                  </div>
