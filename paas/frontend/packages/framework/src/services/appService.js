...
class AppService {
  ...
  getApplicationSettings = (routes, userInfo) => {
    ...
    let asRole =
      cacheRepository.getRole(app) || (userInfo.roles[0] && userInfo.roles[0].roleId) || guestRole
    //若角色id非访客且不在拥有角色内,则直接赋值为访客角色
    if (
      asRole !== guestRole &&
      userInfo.roles.length &&
      userInfo.roles.filter((role) => role.roleId === asRole).length === 0
    ) {
      asRole = guestRole
    }
    ...
    return this.getAppInitData().then((respDatas) => {
      ...
      currentProduct.productName =
        currentProduct.nameCn || currentProduct.appName || currentProduct.name
      ...
      const genAccessRoutes = function (remoteRoutes) {
        remoteRoutes.sort(function (a, b) {
          return a.seq - b.seq
        })
        remoteRoutes.map((route) => {
          ...
          if (path && componentMapping[path]) {
            if (
              route.serviceType === 'mon' &&
              route.type === 'buildIn' &&
              route.config.defaultMonId
            ) {
            ...
            Object.assign(
              ...
              {
                icon: route.icon || componentMapping[path].icon,
                seq: route.seq || componentMapping[path].seq,
              },
            )
          }
          if (route.children) {
  ...
  postWorkspaces(params, namespaceId, stageId) {
    return httpClient.put(
      `gateway/v2/foundation/appmanager/profile?namespaceId=${namespaceId}&stageId=${stageId}`,
      params,
    )
  }
  isLogined() {
  ...
  desktopKeySearch(userEmpId, category, query, type, currentPage = 1, size = 10) {
    return httpClient.get(
      `gateway/v2/foundation/kg/sreworkssearch/query/query_simple_nodes_from_size?__userEmpId=${userEmpId}&category=${category}&query=${query}&type=${type}&page=${currentPage}&size=${size}`,
    )
  }
  testSearchService() {
