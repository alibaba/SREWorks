...
import { Layout, Avatar, PageHeader, Statistic, Descriptions, Menu, Row, Col } from 'antd'
import { Route, Redirect, Link, withRouter } from 'dva/router'
    ...
    if (topMenuType === 'link') {
      return (
        <div className="top-link-menu" style={{ marginTop: 8, paddingLeft: 3 }}>
          {routeData &&
            routeData.children &&
            routeData.children.map(({ path, name, icon, label, config }, key) => {
              ...
              if (path) {
                return (
                  <div key={path} className="link-item">
                    <a style={selectKeys.includes(path) ? { fontWeight: '700' } : {}}>
                      <Link
                        to={{
                          pathname: path,
  ...
  if (routeData.children && routeData.children.length > 0) {
    ...
    return (
      ...
      >
        {routeData &&
          routeData.children &&
          routeData.children.map(
            ({ path, children = [], name, icon, component, label, layout, config }, key) => {
              if (config && (config.hidden === true || config.hidden === 'true')) {
                return null
              }
              ...
              if (children && children.length > 0) {
                return (
                  ...
                  >
                    {children &&
                      children.map(
                        ({ path, children = [], name, icon, component, label, config }, key) => {
                          if (config && (config.hidden === true || config.hidden === 'true'))
                            return null
                          if (path) {
                            return (
...
class PageHeaderLayout extends React.Component {
  constructor(props) {
    ...
    const genRoute = function (routeData) {
      ...
      let displayChildren = children.filter((child) => child.config && !child.config.hidden),
        defaultChildren
          ...
          path &&
            ...
            routeItem.push(
              <Route
                ...
                key={'_' + path}
                component={() => (
                  <Redirect to={defaultChildren.path + '?' + util.objectToUrlParams(pageParams)} />
                )}
              />,
            )
        }
    ...
    routeData &&
      routeData.children &&
      routeData.children.forEach((moduleData) => {
        genRoute(moduleData)
      })
  }
      ...
      <div className="widget-layout-page-content" key={routeData.nodeId || ''}>
        <PageHeader
          className={`common-border brief-page-content globalBackground ${
            title ? '' : 'brief-header-no-title'
          }`}
          style={{
            paddingTop: title ? 32 : 0,
            zIndex: 1,
            position: 'relative',
            marginTop: pageLayoutType === 'FLUID' ? '-12px' : '-20px',
            marginLeft: '-15.4vw',
            marginRight: '-15.4vw',
            paddingLeft: '15.3vw',
            paddingRight: '15.3vw',
                      ...
                      return (
                        <Col>
                          <Statistic
                            title={item.label}
                            suffix={item.unit}
                            value={_.get(item.dataIndex, widgetData)}
                          />
                        </Col>
                      )
                    })}
        ...
        <Layout
          className="brief-page-content"
          style={{
            overflow: 'auto',
            paddingLeft: paddingInner + 'vw',
            paddingRight: paddingInner + 'vw',
            background: 'transparent',
          }}
        >
          {leftSiderMenu && <Sider style={{ marginRight: 8 }}>{leftSiderMenu}</Sider>}
          <Content style={{ margin: '-8px 0px 0' }}>{routeItem}</Content>
        </Layout>
      </div>
    )
  }
