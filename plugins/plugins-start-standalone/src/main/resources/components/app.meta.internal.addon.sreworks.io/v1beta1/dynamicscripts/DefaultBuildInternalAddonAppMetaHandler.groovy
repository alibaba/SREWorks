 ...
 */
class DefaultBuildInternalAddonAppMetaHandler implements BuildComponentHandler {

    private static final Logger log = LoggerFactory.getLogger(DefaultBuildInternalAddonAppMetaHandler.class)
     ...
     * 当前内置 Handler 名称
     */
    public static final String NAME = "app.meta.internal.addon.sreworks.io/v1beta1/build"
    ...
    private static final String TEMPLATE_INTERNAL_ADDON_APPMETA_FILENAME = "default_internal_addon_appmeta.tpl"
    ...
    LaunchBuildComponentHandlerRes launch(BuildComponentHandlerReq request) {
        ...
        // 准备参数并导出数据
        def optionMap = appOptionService.getOptionMap(request.getAppId())
        def data = new JSONObject()
        data.put("options", optionMap)
        FileUtils.writeStringToFile(exportPath.toFile(), data.toJSONString(), Charset.defaultCharset())

        // 创建 meta.yaml 元信息存储到 packageDir 顶层目录中
        def jinjava = JinjaFactory.getJinjava()
        def template = getTemplate(TEMPLATE_INTERNAL_ADDON_APPMETA_FILENAME)
