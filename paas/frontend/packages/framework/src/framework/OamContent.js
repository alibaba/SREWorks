...
class OamContent extends React.Component {
  ...
  loadNodeFilters = (nodeId) => {
    let defaultFilterPath = [],
      { nodeConfig = {} } = this.props
    oamTreeService.getNodeFilters(nodeId).then((filterData) => {
      let defaultFilter = false,
        defalutNodeList = [],
        filterNodeIds = [],
        urlSearchParams = util.getUrlParams(),
        allExistMapping = {}
      ...
      if (filterData.length > 0) {
        ...
        let nodePath = [],
          filterName = defalutNodeList[defalutNodeList.length - 1].name,
          { filterDefaultValue, filterMode } = nodeConfig
      ...
      .then((dataSet) => {
        ...
        if (stepActions.length) {
          stepActions.forEach((sa) => {
            let stepNameList = sa.config.steps.split(','),
              stepList = []
        ...
        if (window.location.hash.includes(this.props.location.pathname)) {
          ...
          let newRegion = nodeParams.meta_region || nodeParams.region,
            oldRegion = urlObject.region
  ...
  render() {
    let { menuRouteData, urlRequireNodeId, match, nodeConfig = {} } = this.props,
      { needCheckRoot, rootTabs, hasChecked, inActionPanelTab } = this.state,
      urlSearchParams = util.getUrlParams()
    //解决树根节点上挂载有页面的场景
    ...
    return (
      <Spin spinning={loading}>
        <OamContentMenuBar
          ...
          openAction={this.handleOpenAction}
          hideActionHistory={
            actions.filter((act) => ['JOB', 'API', 'FLOW'].includes(act.config.actionType))
              .length === 0 || nodeConfig.hideActionHistory === true
          }
        />
        <OamCustomActionBar
          key={openCount}
          ...
          actions={actions}
          nodeParams={nodeParams}
          currentActionMix={currentActionMix}
