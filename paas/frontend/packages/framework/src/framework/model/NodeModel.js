...
export default class NodeModel {
  ...
  initFromJson(nodeData) {
    ...
    this.pageModel = nodeData.pageData
      ? new PageModel(nodeData.pageData)
      : PageModel.CREATE_DEFAULT_INSTANCE()
  ...
  load(stageId) {
    if (this.nodeId) {
      return Promise.all([
        service.getMainPage(this.nodeId, stageId),
        service.getElements(this.nodeId, stageId),
      ]).then((result) => {
        let blocks = [],
          forms = []
        result[1].forEach((item) => {
          let { config, ...other } = item,
            itemData = {}
          itemData = {
            ...other,
            ...config,
  ...
  getBlock(blockKey) {
    return this.blocks.filter(
      (blockMeta) => blockMeta.elementId === blockKey || blockMeta.name === blockKey,
    )[0]
  }
