...
export default class WidgetCard extends React.Component {
  constructor(props) {
    ...
    this.dataSource =
      widgetModel && widgetModel.isVisible(nodeParams) ? widgetModel.getDataSource() : null
    this.state = {
      loading: true,
      close: false,
      fold: false,
  ...
  shouldComponentUpdate(nextProps, nextState) {
    let { widgetModel, nodeParams, cardHeight, location = {} } = this.props,
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (widgetModel.needReload(nodeParams, nextProps.nodeParams) && this.dataSource) {
      if (
        widgetModel &&
        widgetModel.type &&
        Constants.EXCLUDE_COMP_TYPES.includes(widgetModel.type) &&
        widgetModel.dataSourceMeta &&
        widgetModel.dataSourceMeta.api
      ) {
        this.setState({
          runtimeConfig: widgetModel.getRuntimeConfig(
            Object.assign({}, this.state.widgetData, nextProps.nodeParams),
          ),
        })
        return false
      }
      ...
      this.dataSource.query(nextProps.nodeParams).then((widgetData) => {
        this.setState({
          widgetData: widgetData,
          runtimeConfig: widgetModel.getRuntimeConfig(
            Object.assign({}, widgetData, nextProps.nodeParams),
          ),
          refreshLoading: false,
        })
      })
  ...
  render() {
    let {
      ...
      { loading, dataLoading, widgetData, refreshLoading, runtimeConfig, close, fold } = this.state
    let headerExist = false
    if (!widgetModel.isVisible(nodeParams) || close) {
      return <div />
    }
    if (loading || dataLoading || !widgetModel.isReady(nodeParams))
      return <Spin wrapperClassName="abm_frontend_widget_component_spin" />
    const WidgetComponent = this.WidgetComponent
    let { title } = widgetModel.config,
      cardContent,
      toolbarItem = null,
    ...
    if (Constants.FILTERS.includes(widgetModel.type)) {
      ...
      if (window['REMOTE_VUE_LIST'].includes(widgetModel.type) && VueWrapper) {
        let comp = (window[widgetModel.type] && window[widgetModel.type][widgetModel.type]) || (
          <div>未定义组件</div>
        )
        cardContent = (
          <VueWrapper
      ...
      cardContent = (
        <OamWidget
          {...otherProps}
          widget={{ type: widgetModel.type, config: runtimeConfig }}
          widgetConfig={runtimeConfig}
          widgetData={widgetData}
        />
      )
    ...
    if (widgetModel.hasToolbar()) {
      let modelDef =
        (nodeModel.getBlocks() &&
          nodeModel
            .getBlocks()
            .filter((block) => block.elementId === widgetModel.toolbar.filter)[0]) ||
        {}
      if (
        modelDef['elements'] &&
        modelDef['elements'][0]['config'] &&
        modelDef['elements'][0]['config']['rows'] &&
        modelDef['elements'][0]['config']['rows'][0] &&
        modelDef['elements'][0]['config']['rows'][0]['elements']
      ) {
    ...
    let margin =
      mode === Constants.WIDGET_MODE_EDIT
        ? Constants.WIDGET_DEFAULT_MARGIN * 2
        : -Constants.WIDGET_DEFAULT_MARGIN * 0.5
    let calcHeight =
      this.gridPos.h * (Constants.WIDGET_DEFAULT_ROW_HEIGHT + Constants.WIDGET_DEFAULT_MARGIN) -
      Constants.WIDGET_DEFAULT_MARGIN -
      margin
        ...
        <span>
          <a
            style={{
              color:
                headerColor &&
                (this.wrapperType === Constants.CARD_WRAPPER_DEFAULT ||
                  this.wrapperType === Constants.CARD_WRAPPER_ADVANCED)
                  ? '#fafafa'
                  : undefined,
              position: 'relative',
              top: 7,
    ...
    if (
      filterConfig.title &&
      filterConfig.title === 'TAB过滤项' &&
      filterConfig.tabPosition &&
      filterConfig.tabPosition === 'top-left'
    ) {
      toolbarLeft = (
        <ToolBar
          {...this.props}
      ...
      if (foldHandle || hiddenHandle) {
        toolbarRight = (
          <div style={{ display: 'flex', justifyContent: 'space-around' }}>
            {toolbarItem && (
              <div style={{ marginRight: foldHandle || hiddenHandle ? 8 : 0 }}>{toolbarItem}</div>
            )}
    ...
    if (
      this.wrapperType === Constants.CARD_WRAPPER_TRANSPARENT ||
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', justifyContent: 'flex-start' }}>
              {title && (
                <h2 style={{ fontSize: 16, marginRight: '12px' }}>
                  <JSXRender jsx={title} />
                </h2>
              )}
              {toolbarLeft}
                ...
                style={Object.assign(
                  {
                    overflow: widgetModel.config.gridPos ? 'auto' : 'none',
                    display: fold ? 'none' : undefined,
                    height: cardHeight === 'auto' ? 'auto' : calcHeight - 54,
                    padding: 0,
                  },
