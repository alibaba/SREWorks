...
public final class Tools {
    ...
    public static String processTemplate(String template, Map<String, Object> params) {
        Object object = processTemplateObject(template, params);
    ...
    public static Object processTemplateObject(Object template, Map<String, Object> params) {

        template = JSONObject.toJSON(template);
        if (template instanceof String) {
            try {
                template = JSONObject.parse((String) template);
                return processTemplateObject(template, params);
            } catch (Exception e) {
                return processTemplateString((String) template, params);
            }
        } else if (template instanceof JSONObject) {
            JSONObject jsonObject = (JSONObject) template;
            for (String key : jsonObject.keySet()) {
                jsonObject.put(key, processTemplateObject(jsonObject.get(key), params));
            }
            return jsonObject;
        } else if (template instanceof JSONArray) {
            JSONArray jsonArray = (JSONArray) template;
            for (int index = 0; index < jsonArray.size(); index++) {
                jsonArray.set(index, processTemplateObject(jsonArray.get(index), params));
            }
            return jsonArray;
        } else {
            return template;
    ...
    public static Object getValueByPath(String path, Object object) {
        object = JSONObject.toJSON(object);
        if (object instanceof JSONArray) {
            JSONArray jsonArray = (JSONArray) object;
            String startNode = path.split("\\.")[0];
            String releasePath = path.replaceFirst(startNode, "");
            releasePath = StringUtils.strip(releasePath, "\\.");
            if (!StringUtils.isNumeric(startNode)) {
                return null;
            } else {
                int index = Integer.parseInt(startNode);
                if (index >= jsonArray.size()) {
                    return null;
                } else if (StringUtils.isEmpty(releasePath)) {
                    return jsonArray.get(index);
                } else {
                    return getValueByPath(releasePath, jsonArray.get(index));
                }
            }
        } else if (object instanceof JSONObject) {
            return JSONPath.read(JSONObject.toJSONString(object), path);
        } else {
            return null;
        }
    }

    public static JSONObject analysisCronExpression(String cronExpression) throws ParseException {
        cronExpression = cronExpression.trim() + " ?";
        JSONObject retJson = new JSONObject();
        CronExpression ce = new CronExpression(cronExpression);
        Date currTime = new Date();
        Date nextValidTime = ce.getNextValidTimeAfter(currTime);
        Date nextNextValidTime = ce.getNextValidTimeAfter(nextValidTime);
        retJson.put("startPeriod", nextValidTime.getTime() / 1000 - currTime.getTime() / 1000);
        retJson.put("interval", nextNextValidTime.getTime() / 1000 - nextValidTime.getTime() / 1000);
        return retJson;
    }
