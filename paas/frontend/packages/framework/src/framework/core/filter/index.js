...
import { util, safeEval } from '@sreworks/shared-utils'
...
class Filter extends Component {
  ...
  changeVisilbleFromExpression = (props) => {
    ...
    let { visibleExpression } = widgetConfig,
      globalParams = Object.assign({}, nodeParams, userParams, actionParams)
    let isVisable = true
    try {
      if (visibleExpression && visibleExpression.length > 4) {
        if (visibleExpression.includes('$')) {
          visibleExpression = util.renderTemplateString(visibleExpression, globalParams)
        }
  ...
  render() {
    const { actionData, visible } = this.state,
      { mode, widgetModel } = this.props
    if (!actionData) {
      return (
        <div
          style={{
