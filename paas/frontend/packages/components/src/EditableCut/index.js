...
class EditableTable extends React.Component {
  ...
  render() {
    let { form, enableAdd, enableScroll, isCanRemove, ...tableProps } = this.props,
            ...
            if (editingRow && record.__edit_key__ === editingRow.__edit_key__) {
              return (
                <Input
                  ...
                  name={name}
                  placeholder="请输入编辑值"
                  onChange={(e) => this.submitInputValue(e, record.__edit_key__)}
                />
              )
            }
            return text
        ...
        if (editingRow && editingRow.__edit_key__ === record.__edit_key__) {
          return (
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
              <a onClick={this.handleSave} style={{ marginRight: 8 }}>
