...
export default class TabsRender extends Component {
  ...
  render() {
    ...
    if (
      ...
      Object.keys(widgetConfig.toolbar).length > 0 &&
      widgetConfig.hasWrapper &&
      widgetConfig.hasWrapper === 'none'
    ) {
      toolbarItem = <ToolBar {...this.props} widgetConfig={widgetConfig} />
    }
    return (
      <div className="tab-panel-widgets">
        <Tabs
          renderTabBar={(props, DefaultTabBar) => {
            return (
              <div className={tabType === 'capsule' ? 'tab-capsule' : ''}>
                {tabType === 'capsule' && (
                  <span style={{ float: 'left', marginRight: 5, marginTop: 5, fontSize: 14 }}>
                    {title}
                  </span>
                )}
                <DefaultTabBar {...props} />
          ...
          tabBarExtraContent={toolbarItem}
          centered={centered}
