...
class SimpleTable extends React.Component {
  ...
  getApiConf = () => {
    let apiConf = _.get(this.props.mode, 'config.api'),
  ...
  componentWillMount() {
    ...
    columns = mode.config.columns.map((mc) => {
      ...
      if (mc.type === 'ACTION') {
        ...
        mc.render = (text, record) =>
          record.rowActions ? (
            <ActionsRender
              {...record.rowActions}
              nodeParams={this.getNodeParams()}
              record={record}
              openAction={this.handleOpenRowAction}
            />
          ) : (
            <span />
          )
    ...
    } else if (itemToolbar && itemToolbar.actionList && itemToolbar.actionList.length) {
      //增加新的可视化row toolbar适配
      hasOper = true
      columns.push({
        title: localeHelper.get('operation', '操作'),
        key: '_oper',
        ...
        render: (text, record) => (
          <DefaultItemToolbar
            ...
            itemData={record}
            actionParams={Object.assign({}, this.props.actionParams, record)}
      ...
      columns.forEach((c) => {
        if (filters.includes(c.dataIndex)) {
          dispalyItems.push({
            ...
            required: false,
            label: c.title,
            inputTip: localeHelper.get('myProject.searchHint', '输入{title}后按 Enter 键进行查询', {
              title: c.title,
            }),
          })
        }
    ...
    let noPaging =
      paging === 'false' ||
      paging === false ||
      (api && (api.paging === 'false' || api.paging === false))
    if (dynamicColumnsUrl) {
      httpClient
        .get(util.renderTemplateString(dynamicColumnsUrl, this.getNodeParams()))
        .then((dcs) => {
          let oc = null
          if (hasOper) {
            oc = columns.pop()
    ...
    if (
      noPaging &&
      ((widgetConfig.dataSourceMeta && widgetConfig.dataSourceMeta.api) || (apiConf && apiConf.url))
    ) {
      this.loadAllData(api).then((data) => {
        let dataIndex = mode.config.dataIndex,
          tdata = (data && data.items) || data || []
  ...
  componentDidUpdate(prevProps) {
    let { mode, parameters, nodeParams } = this.props,
      { apiConf } = this.state
    if (apiConf && apiConf.url && !_.isEqual(prevProps.nodeParams, nodeParams)) {
      //前端api方式配置
      let api = apiConf
    ...
    } else if (!_.isEqual(prevProps.nodeParams, nodeParams)) {
      //datasource形式
  ...
  handleOpenRowAction = (name, record, callback) => {
    ...
    let outputs = mode.config.outputs,
      params = { ...record }
    if (outputs) {
  ...
  render() {
    ...
    let confParams = Object.assign(_.get(apiConf, 'params') || {}, util.getUrlParams(), {
        ___refresh_timestamp: nodeParams.___refresh_timestamp,
      }),
      allParams = this.getNodeParams()
    let tableParams = {
      nodeId: nodeId,
      elementId: mode.elementId,
      parameters: parameters,
    ...
    if (title || headerActions || headerLinks) {
      if (title) {
        titleItem = (
          <div key="__title">
            <h4 style={{ fontSize: 13 }} key="__title">
              {title}
            </h4>
          </div>
        )
      ...
      dynamiConf.title = () => {
        return (
          <div key="__dtitle" style={{ display: 'flex', justifyContent: 'space-between' }}>
            <div>{title ? titleItem : headerActionItem}</div>
      ...
      if (dynamiConf.rowSelection) {
        slectAllCheckbox = (
          <Checkbox
            ...
            indeterminate={footIndete}
            checked={footChecked}
            onChange={this.handleFooterChecked}
          />
        )
      }
      footerActionItem = (
    ...
    if (expandedRow && Object.keys(expandedRow).length) {
      dynamiConf.expandedRowRender = (record) => (
        <OamWidget
          openAction={openAction}
          widget={mode.__template.config.expandedRow}
          parameters={parameters}
    ...
    let tableKey = 'table_reload_' + reloadCount + data && data.length,
      { wrapper } = widgetConfig,
      dyClass = 'inner-table globalBackground'
    if (size !== 'small') {
